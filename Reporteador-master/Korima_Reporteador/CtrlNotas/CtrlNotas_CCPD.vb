...
Public Class CtrlNotas_CCPD
    ...
    Private Function MesLetra(ByVal Mes As Integer) As String
        ...
        End Select
    End Function
    Dim reporte As NOTA_CCPD

    Private Sub SimpleButton1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SimpleButton1.Click
        Me.Cursor = Cursors.WaitCursor
        SplitContainerControl1.Collapsed = True
        reporte = New NOTA_CCPD()
        Dim orden As Integer
        If RadioButton1.Checked = True Then orden = 0
        If RadioButton2.Checked = True Then orden = 1
        Dim printTool As New ReportPrintTool(reporte)

        Dim conection As New SqlConnection(cnnString)
        Dim prmOrden As New SqlParameter("Orden", orden)
        Dim command As New SqlCommand("SP_RPT_K2_Conceptos_Percepciones_Deducciones", conection)
        command.CommandType = CommandType.StoredProcedure
        command.Parameters.Add(prmOrden)
        Dim adapter As New SqlDataAdapter(command)
        Dim ds As New DataSet()
        ds.EnforceConstraints = False
        adapter.Fill(ds, "SP_RPT_K2_Conceptos_Percepciones_Deducciones")
        reporte.DataSource = ds

        reporte.DataAdapter = adapter
        reporte.DataMember = "SP_RPT_K2_Conceptos_Percepciones_Deducciones"

        'Firmas 
        Dim adapterC As SqlClient.SqlDataAdapter
        adapterC = New SqlClient.SqlDataAdapter("SELECT * FROM VW_RPT_K2_Firmas where Formato = 'Catálogo de Percepciones y Deducciones' and (Nombre1 <>'' or Puesto1 <> '' or Nombre2 <>'' or Puesto2<>'') Order by Orden ", cnnString)
        ...
        With reporte
            .lblRptNombreEnte.Text = "Entidad Federativa: " + pRPTCFGDatosEntes.Nombre
            .lblRptNombreReporte.Text = "Catálogo de Conceptos de Percepciones y Deducciones"
            ...
            Dim cmd As New SqlCommand("SELECT TOP(1) T_Firmas.CodigoISO FROM C_Formatos JOIN T_Firmas ON C_Formatos.IdFormato = T_Firmas.IdFormato  Where Formato= 'Catálogo de Percepciones y Deducciones'", New SqlConnection(cnnString))
            cmd.Connection.Open()
