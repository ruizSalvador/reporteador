/****** Object:  UserDefinedFunction [dbo].[RPT_Balanza_Comprobacio_Anterior]    Script Date: 05/07/2013 09:23:08 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[RPT_Balanza_Comprobacio_Anterior]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
DROP FUNCTION [dbo].[RPT_Balanza_Comprobacio_Anterior]
GO
...
WHERE     (dbo.C_Contable.TipoCuenta <> 'X') AND ((RIGHT(dbo.C_Contable.NumeroCuenta, 5) = '00000') OR (RIGHT(dbo.C_Contable.NumeroCuenta, 6) = '000000')) AND (LEFT(dbo.C_Contable.NumeroCuenta, 1) > '0') AND 
                      (LEFT(dbo.C_Contable.NumeroCuenta, 1) > '3' AND LEFT(dbo.C_Contable.NumeroCuenta, 1) < '6') and dbo.T_SaldosInicialesCont.Mes = @mes and
