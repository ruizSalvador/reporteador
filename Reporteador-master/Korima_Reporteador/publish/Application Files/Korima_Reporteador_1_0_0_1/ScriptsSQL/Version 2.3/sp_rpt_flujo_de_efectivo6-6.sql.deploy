/****** Object:  StoredProcedure [dbo].[sp_rpt_flujo_de_efectivo6-6]    Script Date: 07/25/2013 11:41:56 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_rpt_flujo_de_efectivo6-6]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[sp_rpt_flujo_de_efectivo6-6]
GO
...
CREATE PROCEDURE [dbo].[sp_rpt_flujo_de_efectivo6-6]
...
(Select * From T_SaldosInicialesCont Where Mes = @periodo And [Year] = @ejercicio) Saldos
...
Select NumeroCuenta, NombreCuenta, CargosSinFlujo, AbonosSinFlujo, TotalCargos, TotalAbonos,
      ...
      End as SaldoAcreedor
From C_Contable LEFT JOIN 
(Select * From T_SaldosInicialesCont Where Mes = @periodo And [Year] = @ejercicio - 1) Saldos
On C_Contable.IdCuentaContable = Saldos.IdCuentaContable
Where TipoCuenta <> 'X' 
Order By NumeroCuenta
...
Select nombre, (ISNULL(SaldoActual,0)/1000) as SaldoActual, (ISNULL(SaldoAnterior,0)/1000) as SaldoAnterior,
