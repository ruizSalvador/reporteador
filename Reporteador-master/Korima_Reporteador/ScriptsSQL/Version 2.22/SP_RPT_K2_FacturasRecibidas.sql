...
as
DECLARE @Tabla TABLE (Fecha datetime,SubTotal decimal(15,2),IVA decimal(15,2),Total decimal(15,2),Folio Int,Observaciones varchar(max),RazonSocial varchar(max), RFC varchar(max), Factura varchar(max), Status varchar(15))
...
Where (T_RecepcionFacturas.FechaFactura >= (@Fecha-1) and T_RecepcionFacturas.FechaFactura <= @FechaFin ) and C_Proveedores.RazonSocial =@Proveedor AND (T_RecepcionFacturas.Estatus <> 'N' and T_SolicitudCheques.Estatus <> 'N')
...
C_Proveedores.RazonSocial,'R.F.C. '+C_Proveedores.RFC as RFC,D_Viaticos.Referencia as Factura, 'Pagado' as Status
FROM T_Viaticos
...
Where (T_RecepcionFacturas.FechaFactura >= (@Fecha-1) and T_RecepcionFacturas.FechaFactura <= @FechaFin ) AND (T_RecepcionFacturas.Estatus <> 'N' and T_SolicitudCheques.Estatus <> 'N')
UNION
Select T_Viaticos.Fecha, SUM(isnull(D_Viaticos.Importe,0)) as SubTotal, SUM(isnull(D_Viaticos.IVA,0)) as IVA,SUM(isnull(D_Viaticos.Importe,0))+SUM(isnull(D_Viaticos.IVA,0))as Total,T_Viaticos.Folio,T_Viaticos.Justificacion as Observaciones,
C_Proveedores.RazonSocial,'R.F.C. '+C_Proveedores.RFC as RFC,D_Viaticos.Referencia as Factura, 'Pagado' as Status
FROM T_Viaticos
...
--EXEC SP_RPT_K2_FacturasRecibidas '20150101','20151231',''
