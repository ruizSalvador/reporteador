/****** Object:  StoredProcedure [dbo].[sp_rpt_flujo_de_efectivo2RedondeoHor]    Script Date: 12/03/2012 17:30:04 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_rpt_flujo_de_efectivo2RedondeoHor]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[sp_rpt_flujo_de_efectivo2RedondeoHor]
GO

-- Exec sp_rpt_flujo_de_efectivo2RedondeoHor 2015,4,4,2015,0,1,1,1
CREATE PROCEDURE [dbo].[sp_rpt_flujo_de_efectivo2RedondeoHor]
...
From C_Contable LEFT JOIN 
(Select IdCuentaContable, Year,Mes,SUM(isnull(TotalCargos,0)) as TotalCargos,SUM(isnull(TotalAbonos,0))as TotalAbonos,SUM(isnull(CargosSinFlujo,0)) as CargosSinFlujo,SUM(isnull(AbonosSinFlujo,0)) as AbonosSinFlujo 
From T_SaldosInicialesCont Where ( Mes = @PeriodoFin) And [Year] = @ejercicio2 group by IdCuentaContable,Year,Mes) Saldos
On C_Contable.IdCuentaContable = Saldos.IdCuentaContable
Where TipoCuenta <> 'X' 
Group By NumeroCuenta,NombreCuenta,TipoCuenta
Order By NumeroCuenta
...
Union All
Select 'OTROS ORIGENES DE OPERACIÓN'	, a.TotalAbonos - a.TotalCargos , b.TotalAbonos-b.TotalCargos , 0, 'X',19
FROM @eActual a inner join @eAnterior b on a.nocuenta = b.nocuenta
...
UPDATE @report set SaldoAnterior= ( Select SUM(b.SaldoAcreedor) FROM @eAnterior b inner join @eActual a on a.nocuenta = b.nocuenta 
Where SUBSTRING(a.nocuenta ,1,4) In ('8150') And   -- BR
