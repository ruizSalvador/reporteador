...
--Exec SP_RPT_AnaliticoPorProveedorContable '01/01/2016','20161107',1402,0,0,1
...
Group by TC.IdChequesAgrupador, TP.Folio, CP.RazonSocial, TP.TotalGral, TP.Estatus, TAP.IdPoliza, TRF.Serie, TRF.Factura, TRF.IdPoliza,
	...
	TC.IdPolizaPresupuestoPagado, TC.Entregado
	...
	If @Cancelados = 1
			Begin
				Select  
				  ...
				  ,Tipo 
				  ,CASE
					WHEN lag(OSOC) over(order by (select null)) = OSOC
					THEN 'X'
					ELSE OSOC
					END  as ForSum
					from @tabla order by Tipo
			...
			Begin
				...
				Select  
				  ...
				  ,CASE
					...
					ELSE OSOC
					END  as ForSum
				from @tabla where Estatus <> 'Cancelado' order by Tipo
			End
End
