/****** Object:  StoredProcedure [dbo].[SP_RPT_K2_AuxiliarMayorMovimientos]    Script Date: 11/05/2013 09:05:50 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_RPT_K2_AuxiliarMayorMovimientos]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_RPT_K2_AuxiliarMayorMovimientos]
GO
...
CREATE PROCEDURE [dbo].[SP_RPT_K2_AuxiliarMayorMovimientos] 
...
set @iCuentaFin= CONVERT(bigint,REPLACE(@CuentaFin,'-',''))
 ...
 Declare @tabla as table (
	  ...
	  ,TipoDeMovimiento varchar(50)
	  ,Beneficiario varchar (100)
	  ,Rfc varchar(13)
	  ,SolicitudEgresos int
	  ,Estatus varchar(50)
...
convert(bigint,replace (C_Contable.NumeroCuenta,'-','') ) as iNumeroCuenta,
 CASE T_SolicitudCheques.PorDeposito 
	WHEN 0 THEN 'Cheques'
	WHEN 1 THEN 'Transferencias Electrónicas'
	ELSE Null
...
CASE --T_Cheques.Status 
 WHEN T_Cheques.Status ='D' THEN 'Aplicados'
 WHEN T_Cheques.Status ='I' and T_Cheques.Entregado = 0 THEN 'Impresos No Entregados'
 WHEN T_Cheques.Status ='I' and T_Cheques.Entregado = 1 THEN 'Impresos Entregados'
 WHEN T_Cheques.Status ='N' THEN 'Cancelados'
 ELSE Null
 END as Estatus,
...
D_Polizas.Concepto,
convert(bigint,replace (C_Contable.NumeroCuenta,'-','') ) as iNumeroCuenta,
 T_SolicitudCheques.PorDeposito as TipoDeMovimiento, 
 T_Cheques.Beneficiario, 
CASE
		WHEN T_Cheques.IdProveedor = 0 THEN C_Empleados.Rfc 
	    ELSE C_Proveedores.RFC 
...
Case C_Contable .TipoCuenta  
      ...
      convert(bigint,replace (C_Contable.NumeroCuenta,'-','') ) as iNumeroCuenta,
	  Null as TipoDeMovimiento,
	  Null as  Beneficiario, 
	  Null as  Rfc,
	  Null as  SolicitudEgresos,
	 Null as Estatus,
	 Null as FechaImpresion
...
Exec SP_CFG_LogScripts 'SP_RPT_K2_AuxiliarMayorMovimientos'
GO
--EXEC SP_RPT_K2_AuxiliarMayorMovimientos '','',2015,1,5,1,'','',0
