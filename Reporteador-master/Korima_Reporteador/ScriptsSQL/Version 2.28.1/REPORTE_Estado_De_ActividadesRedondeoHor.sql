/****** Object:  StoredProcedure [dbo].[REPORTE_Estado_De_ActividadesHor]    Script Date: 07/23/2014 09:45:55 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[REPORTE_Estado_De_ActividadesRedondeoHor]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[REPORTE_Estado_De_ActividadesRedondeoHor]
GO

/****** Object:  StoredProcedure [dbo].[REPORTE_Estado_De_ActividadesRedondeoHor]    Script Date: 03/10/2014 09:45:55 ******/
SET ANSI_NULLS ON
GO

SET QUOTED_IDENTIFIER ON
GO
--exec REPORTE_Estado_De_ActividadesRedondeoHor 5,5,2015,2015,1,1,1,2
CREATE PROCEDURE [dbo].[REPORTE_Estado_De_ActividadesRedondeoHor]
@mes smallint,
@mes2 smallint,
@año smallint,
@año2 smallint,
...
Declare @Tmp_BalanzaDeComprobacion2 TABLE(
NumeroCuenta varchar(max),
NombreCuenta varchar(max),
CargosSinFlujo decimal(15,2),
AbonosSinFlujo decimal(15,2),
...
Insert Into @TablaTitulos values('417'+REPLICATE('0',@Estructura1-3),'              INGRESOS POR VENTA DE BIENES Y PRESTACIÓN DE SERVICIOS',2)
...
Insert Into @TablaTitulos values('421'+REPLICATE('0',@Estructura1-3),'              Participaciones, Aportaciones, Convenios, Incentivos Derivados de la Colaboración Fiscal y Fondos Distintos de Aportaciones',2)
...
Insert Into @TablaTitulos values('554'+REPLICATE('0',@Estructura1-3),'              AUMENTO POR INSUFICIENCIA DE ESTIMACIONES POR PÉRDIDA O DETERIORO U OBSOLESCENCIA',2)
...
Insert Into @Tmp_BalanzaDeComprobacion2
Exec REPORTE_Estado_De_ActividadesRedondeo @mes2, @año2,0, 0, 0

Insert Into @Tmp_BalanzaDeComprobacionFinal
Select * from @Tmp_BalanzaDeComprobacion

 Update Final Set Final.SaldoDeudorAnt = tmp.SaldoDeudor FROM  @Tmp_BalanzaDeComprobacionFinal As Final Inner Join @Tmp_BalanzaDeComprobacion2 As tmp ON Final.NumeroCuenta = tmp.NumeroCuenta
 Update Final Set Final.SaldoAcreedorAnt = tmp.SaldoAcreedor FROM  @Tmp_BalanzaDeComprobacionFinal As Final Inner Join @Tmp_BalanzaDeComprobacion2 As tmp ON Final.NumeroCuenta = tmp.NumeroCuenta
