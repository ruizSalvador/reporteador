...
--Seleccion Final
	...
	Orden1 ,
	Orden2,  
	case when (isnull(Saldo,0)> isnull(SaldoANT,0)) then ABS(isnull(Saldo,0)/@division-isnull(SaldoANT,0)/@division) end as Aplicacion,
	case when (isnull(SaldoANT,0)>isnull(Saldo,0)) then ABS(isnull(Saldo,0)/@division-isnull(SaldoANT,0)/@division) end as Origen   
	from @titulos
	End
	Else Begin
		select 
