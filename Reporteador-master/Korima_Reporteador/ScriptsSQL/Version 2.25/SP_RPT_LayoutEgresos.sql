...
--Exec SP_RPT_LayoutEgresos '20171231',2017
...
ISNULL((Select SUM(Ampliaciones) from T_PresupuestoNW TP Inner Join T_SellosPresupuestales TS On TP.IdSelloPresupuestal = TS.IdSelloPresupuestal WHERE LYear = @Ejercicio AND (Mes Between 1 and @mes)),0),
ISNULL((Select SUM(Reducciones) from T_PresupuestoNW TP Inner Join T_SellosPresupuestales TS On TP.IdSelloPresupuestal = TS.IdSelloPresupuestal WHERE LYear = @Ejercicio AND (Mes Between 1 and @mes)),0),
ISNULL((Select SUM(Comprometido) from T_PresupuestoNW TP Inner Join T_SellosPresupuestales TS On TP.IdSelloPresupuestal = TS.IdSelloPresupuestal WHERE LYear = @Ejercicio AND (Mes Between 1 and @mes)),0),
...
SELECT IdSelloPresupuestal, IdAreaResp, IdPartida, IdSubFuncion, IdTipoGasto, IdProyecto, IdFuenteFinanciamiento, IdClasificadorGeografico, Proyecto
  from T_SellosPresupuestales where LYear = @Ejercicio order by IdSelloPresupuestal
  
OPEN Registros
	FETCH NEXT FROM Registros INTO  @IdSello, @IdArea, @IdPartida, @IdSubfuncion, @IdTipoGasto, @IdProyecto, @IdFuente, @IdClasGeo, @Proyecto 
WHILE @@FETCH_STATUS = 0
	...
	(Select SUBSTRING(Clave,1,1) From C_FuenteFinanciamiento Where IDFUENTEFINANCIAMIENTO = @IdFuente),
	(Select SUBSTRING(Clave,2,2) From C_FuenteFinanciamiento Where IDFUENTEFINANCIAMIENTO = @IdFuente),
	...
	ISNULL((Select SUM(Ampliaciones) from T_PresupuestoNW TP Inner Join T_SellosPresupuestales TS On TP.IdSelloPresupuestal = TS.IdSelloPresupuestal WHERE LYear = @Ejercicio AND (MES Between 1 and @mes) and TP.IdSelloPresupuestal = @IdSello),0),
	ISNULL((Select SUM(Reducciones) from T_PresupuestoNW TP Inner Join T_SellosPresupuestales TS On TP.IdSelloPresupuestal = TS.IdSelloPresupuestal WHERE LYear = @Ejercicio AND (MES Between 1 and @mes) and TP.IdSelloPresupuestal = @IdSello),0),
...
Exec SP_CFG_LogScripts 'SP_RPT_LayoutEgresos'
GO