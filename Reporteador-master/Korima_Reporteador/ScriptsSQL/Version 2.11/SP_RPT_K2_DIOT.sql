/****** Object:  StoredProcedure [dbo].[SP_RPT_K2_DIOT]    Script Date: 04/08/2015 10:26:13 ******/
...
AS
Declare @Campo8 as table (IdProveedor int,Importe Decimal(18,4))
insert into @Campo8 
...
ON T_Cheques.IdSolicitudCheque =T_SolicitudCheques.IdSolicitudCheques 
 ...
 ---------
Declare @_Campo8 as table (IdProveedor int,Importe Decimal(18,4))
insert into @_Campo8 
select c8.IdProveedor, SUM (c8.Importe) from @campo8 as c8 group by c8.IdProveedor 
 ---------------------------------------
Declare @Campo14 as table (IdProveedor int,Importe Decimal(18,4))
insert into @Campo14 
select T_Pedidos.IdProveedor ,
...
ON T_Cheques.IdSolicitudCheque =T_SolicitudCheques.IdSolicitudCheques 
 ...
 AND(MONTH(T_OrdenServicio.Fecha)=@mes AND YEAR(T_OrdenServicio.Fecha)=@Ejercicio)
 group by T_OrdenServicio.idproveedor
 Declare @_Campo14 as table (IdProveedor int,Importe Decimal(18,4))
insert into @_Campo14 
select c14.IdProveedor, SUM (c14.Importe) from @campo14 as c14 group by c14.IdProveedor 
--------------------------------------
Declare @Campo18 as table (IdProveedor int,Importe Decimal(18,4))
insert into @Campo18
select T_Pedidos.IdProveedor ,
sum (isnull(d_pedidos.Importe,0) * isnull(T_Pedidos.TipodeCambio,0))
from d_pedidos join 
...
group by T_OrdenServicio.idproveedor
------
Declare @_Campo18 as table (IdProveedor int,Importe Decimal(18,4))
insert into @_Campo18 
select c18.IdProveedor, SUM (c18.Importe) from @campo18 as c18 group by c18.IdProveedor 
--------------------------------------
Declare @Campo19 as table (IdProveedor int,Importe Decimal(18,4))
insert into @Campo19 
select T_Pedidos.IdProveedor ,
sum (d_pedidos.Importe)
from d_pedidos join 
...
ON T_Cheques.IdSolicitudCheque =T_SolicitudCheques.IdSolicitudCheques 
 ...
 group by T_OrdenServicio.idproveedor
 ------
Declare @_Campo19 as table (IdProveedor int,Importe Decimal(18,4))
insert into @_Campo19 
select c19.IdProveedor, SUM (c19.Importe) from @campo19 as c19 group by c19.IdProveedor 
--------------------------------------
Declare @Campo20 as table (IdProveedor int,Importe Decimal(18,4))
insert into @Campo20
select T_Pedidos.IdProveedor ,
sum (isnull(d_pedidos.Importe,0) * isnull(T_Pedidos.TipodeCambio,0))
from d_pedidos join 
...
ON T_Cheques.IdSolicitudCheque =T_SolicitudCheques.IdSolicitudCheques 
 ...
 group by T_OrdenServicio.idproveedor
 ------
Declare @_Campo20 as table (IdProveedor int,Importe Decimal(18,4))
insert into @_Campo20 
select c20.IdProveedor, SUM (c20.Importe) from @campo20 as c20 group by c20.IdProveedor 
--------------------------------------
Declare @Campo21 as table (IdProveedor int,Importe Decimal(18,4))
insert into @Campo21
select T_OrdenServicio.IdProveedor ,
...
ON T_Cheques.IdSolicitudCheque =T_SolicitudCheques.IdSolicitudCheques 
 ...
 group by T_OrdenServicio.idproveedor
--------------------------------------
Declare @Campo22 as table (IdProveedor INT,Importe DECIMAL(18,4))
INSERT INTO @Campo22
