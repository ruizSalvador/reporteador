...
--Exec SP_EstadoEjercicioIngresos_Rubro_Tipo 1,1,1,2018,1,1
...
--
Insert into @Rubro
EXEC SP_EstadoEjercicioIngresos_Rubro @MesInicio, @MesFin, 1, @Ejercicio, @AprAnual,@AmpRedAnual
--
...
IF @AmpRedAnual = 1
	BEGIN
		DECLARE @AmpAnual AS TABLE (Clasificacion varchar(255), Clave varchar(7), AmpliacionesReducciones decimal(18,2))
		INSERT INTO @AmpAnual
		SELECT C_Clas2.Descripcion, C_Clas2.Clave, SUM (isnull(Ampliaciones,0) )+ SUM (isnull(Reducciones,0)) AS AmpliacionesReducciones
		FROM T_PresupuestoFlujo TP LEFT OUTER JOIN C_PartidasGastosIngresos TS ON TP.IdPartida  = TS.IdPartidaGI 
		LEFT OUTER JOIN C_ClasificacionGasto C_Clas ON TS.IdClasificacionGI = C_Clas.IdClasificacionGI 
		LEFT OUTER JOIN C_ClasificacionGasto C_Clas1
		ON C_Clas.IdClasificacionGIPadre = C_Clas1.IdClasificacionGI LEFT OUTER JOIN C_ClasificacionGasto C_Clas2
		...
		UPDATE A SET A.AmpliacionesReducciones = B.AmpliacionesReducciones FROM @Resultado A INNER JOIN @AmpAnual B ON A.Clave = B.Clave
...
IF @MostrarVacios=1
	BEGIN
		...
		isnull(Total_Estimado,0)+isnull(AmpliacionesReducciones,0) as Total_Modificado,
		...
		isnull((Select Sum(Total_Estimado) + Sum(AmpliacionesReducciones) from @Resultado where Grupo2 like '_0'),0) as SumaModificado,
...
GO
Exec SP_CFG_LogScripts 'SP_EstadoEjercicioIngresos_Rubro_Tipo','2.29'
GO
