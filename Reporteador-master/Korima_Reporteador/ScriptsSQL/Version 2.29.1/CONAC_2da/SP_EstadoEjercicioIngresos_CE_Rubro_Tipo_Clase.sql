...
-- Red/Amp Acumuladas
DECLARE @AcumAnual AS TABLE (Clasificacion varchar(255), Clave varchar(7), Estimado decimal(15,2), AmpliacionesReducciones decimal(18,2), Modificado decimal(15,2))
...
IF @AmpRedAnual = 1
	BEGIN
		...
		UPDATE A SET A.AmpliacionesReducciones = B.AmpliacionesReducciones FROM @Resultado A INNER JOIN @AcumAnual B ON A.Clave = B.Clave;	
		 ...
		 UPDATE A SET A.AmpliacionesReducciones = B.AmpliacionesReducciones FROM @Resultado A INNER JOIN @AcumAnual B ON A.Grupo1 = B.Clave;
	END
...
GO
Exec SP_CFG_LogScripts 'SP_EstadoEjercicioIngresos_CE_Rubro_Tipo_Clase','2.29'
GO
