...
--exec SP_RPT_InformeProgramaEstadoEjercicioPresupuestoEGR 1,12,2015,'','','','',9
...
BEGIN
	...
	BEGIN
		...
		--Columna 1 PorcAprobAnual
		Declare @TPorcAprobAnual_9_1 as Table(PorcAprobAnual1 decimal(15,2), Clave varchar (max), Clave4 varchar(Max), ID int)
		insert into @TPorcAprobAnual_9_1
		Select  sum(isnull(TP.Autorizado,0)) as PorcAprobAnual1, cr.CLAVE, cpi.clave as Clave4, cepr.Id 
		FROM T_SellosPresupuestales INNER JOIN T_PRESUPUESTONW TP ON T_SellosPresupuestales.IdSelloPresupuestal = TP.IdSelloPresupuestal
		INNER JOIN C_RamoPresupuestal  CR ON CR.IDRAMOPRESUPUESTAL = T_SellosPresupuestales.IdRamoPresupuestal  INNER JOIN C_EP_Ramo cepr
		ON cepr.Id    = T_SellosPresupuestales.IdProyecto INNER JOIN C_ProyectosInversion cpi ON cpi.PROYECTO = T_SellosPresupuestales.Proyecto  
		...
		Declare @TPorcAprobAnual_9_2 as Table(PorcAprobAnual2 decimal(15,2), Clave varchar (max), Clave4 varchar(Max), ID int)
		...
		--Columna 2 PorcAprobadocAnt
		Declare @TPorcAprobadocAnt_9_1 as Table(PorcAprobadocAnt1 decimal(15,2), Clave varchar (max), Clave4 varchar(Max), ID int)
		insert into @TPorcAprobadocAnt_9_1
		Select  sum(isnull(tp.Autorizado,0)) as PorcAprobadocAnt1, cr.CLAVE, cpi.clave as Clave4, cepr.Id
		FROM T_SellosPresupuestales INNER JOIN T_PRESUPUESTONW TP ON T_SellosPresupuestales.IdSelloPresupuestal = TP.IdSelloPresupuestal
		INNER JOIN C_RamoPresupuestal  CR ON CR.IDRAMOPRESUPUESTAL = T_SellosPresupuestales.IdRamoPresupuestal  INNER JOIN C_EP_Ramo cepr
		...
		Declare @TPorcAprobadocAnt_9_2 as Table(PorcAprobadocAnt2 decimal(15,2), Clave varchar (max), Clave4 varchar(Max), ID int)
		...
		--Columna 3 y 4 NomAprobado
		Declare @TNomAprobado_9_1 as Table(NomAprobado1 decimal(15,2), Clave varchar (max), Clave4 varchar(Max), ID int)
		insert into @TNomAprobado_9_1
		Select  (sum(isnull(tp.Autorizado,0))) as NomAprobado1, cr.CLAVE, cpi.clave as Clave4, cepr.Id
		FROM T_SellosPresupuestales INNER JOIN T_PRESUPUESTONW TP ON T_SellosPresupuestales.IdSelloPresupuestal = TP.IdSelloPresupuestal
		INNER JOIN C_RamoPresupuestal  CR ON CR.IDRAMOPRESUPUESTAL = T_SellosPresupuestales.IdRamoPresupuestal  INNER JOIN C_EP_Ramo cepr
		ON cepr.Id    = T_SellosPresupuestales.IdProyecto INNER JOIN C_ProyectosInversion cpi ON cpi.PROYECTO = T_SellosPresupuestales.Proyecto  
		left JOIN C_PartidasPres  ON C_PartidasPres.IdPartida   = T_SellosPresupuestales.IdPartida  left JOIN C_ClasificadorGeograficoPresupuestal cgp 
		...
		Declare @TNomAprobado_9_2 as Table(NomAprobado2 decimal(15,2), Clave varchar (max), Clave4 varchar(Max), ID int)
		insert into @TNomAprobado_9_2
		Select  sum(isnull(tp.Autorizado,0)) as NomAprobado2, cr.CLAVE, cpi.clave as Clave4, cepr.Id
		FROM T_SellosPresupuestales INNER JOIN T_PRESUPUESTONW TP ON T_SellosPresupuestales.IdSelloPresupuestal = TP.IdSelloPresupuestal
		INNER JOIN C_RamoPresupuestal  CR ON CR.IDRAMOPRESUPUESTAL = T_SellosPresupuestales.IdRamoPresupuestal  INNER JOIN C_EP_Ramo cepr
		ON cepr.Id    = T_SellosPresupuestales.IdProyecto INNER JOIN C_ProyectosInversion cpi ON cpi.PROYECTO = T_SellosPresupuestales.Proyecto  
		left JOIN C_PartidasPres  ON C_PartidasPres.IdPartida   = T_SellosPresupuestales.IdPartida  left JOIN C_ClasificadorGeograficoPresupuestal cgp 
		...
		--Columna 5 PorcPVigenteAnual
		Declare @TPorcPVigenteAnual_9_1 as Table(PorcPVigenteAnual1 decimal(15,2), Clave varchar (max), Clave4 varchar(Max), ID int)
		insert into @TPorcPVigenteAnual_9_1
		Select  (sum(ISNULL(TP.Autorizado,0)) + (sum(ISNULL(TP.Ampliaciones,0)) + sum(ISNULL(TP.TransferenciaAmp,0))) - (sum(ISNULL(TP.Reducciones,0)) 
		+ sum(ISNULL(TP.TransferenciaRed,0)))) as PorcPVigenteAnual1, cr.CLAVE, cpi.clave as Clave4, cepr.Id
		FROM T_SellosPresupuestales INNER JOIN T_PRESUPUESTONW TP ON T_SellosPresupuestales.IdSelloPresupuestal = TP.IdSelloPresupuestal
		INNER JOIN C_RamoPresupuestal  CR ON CR.IDRAMOPRESUPUESTAL = T_SellosPresupuestales.IdRamoPresupuestal  INNER JOIN C_EP_Ramo cepr
		ON cepr.Id    = T_SellosPresupuestales.IdProyecto INNER JOIN C_ProyectosInversion cpi ON cpi.PROYECTO = T_SellosPresupuestales.Proyecto  
		...
		Declare @TPorcPVigenteAnual_9_2 as Table(PorcPVigenteAnual2 decimal(15,2), Clave varchar (max), Clave4 varchar(Max), ID int)
		...
		--Columna 6 PorcPVigenteAnt
		Declare @TPorcPVigenteAnt_9_1 as Table(PorcPVigenteAnt1 decimal(15,2), Clave varchar (max), Clave4 varchar(Max), ID varchar)
		insert into @TPorcPVigenteAnt_9_1
		Select  (sum(ISNULL(TP.Autorizado,0)) + (sum(ISNULL(TP.Ampliaciones,0)) + sum(ISNULL(TP.TransferenciaAmp,0))) - (sum(ISNULL(TP.Reducciones,0)) 
		+ sum(ISNULL(TP.TransferenciaRed,0)))) as PorcPVigenteAnt1, cr.CLAVE, cpi.clave as Clave4, cepr.Id  
		FROM T_SellosPresupuestales INNER JOIN T_PRESUPUESTONW TP ON T_SellosPresupuestales.IdSelloPresupuestal = TP.IdSelloPresupuestal
		INNER JOIN C_RamoPresupuestal  CR ON CR.IDRAMOPRESUPUESTAL = T_SellosPresupuestales.IdRamoPresupuestal  
		INNER JOIN C_EP_Ramo cepr ON cepr.Id    = T_SellosPresupuestales.IdProyecto INNER JOIN C_ProyectosInversion cpi
		ON cpi.PROYECTO = T_SellosPresupuestales.Proyecto  left JOIN C_PartidasPres  ON C_PartidasPres.IdPartida   = T_SellosPresupuestales.IdPartida  
		...
		Declare @TPorcPVigenteAnt_9_2 as Table(PorcPVigenteAnt2 decimal(15,2), Clave varchar (max), Clave4 varchar(Max), ID varchar)
		...
		--Columna 7 y 8 NomPVigente
		Declare @TNomPVigente_9_1 as Table(NomPVigente1 decimal(15,2), Clave varchar (max), Clave4 varchar(Max), ID int)
		insert into @TNomPVigente_9_1
		Select  ((sum(ISNULL(TP.Autorizado,0)) + (sum(ISNULL(TP.Ampliaciones,0)) + sum(ISNULL(TP.TransferenciaAmp,0))) - (sum(ISNULL(TP.Reducciones,0)) 
		+ sum(ISNULL(TP.TransferenciaRed,0)))))  as NomPVigente1, cr.CLAVE, cpi.clave as Clave4, cepr.Id 
		FROM T_SellosPresupuestales INNER JOIN T_PRESUPUESTONW TP ON T_SellosPresupuestales.IdSelloPresupuestal = TP.IdSelloPresupuestal
		INNER JOIN C_RamoPresupuestal  CR ON CR.IDRAMOPRESUPUESTAL = T_SellosPresupuestales.IdRamoPresupuestal  INNER JOIN C_EP_Ramo cepr
		ON cepr.Id    = T_SellosPresupuestales.IdProyecto INNER JOIN C_ProyectosInversion cpi ON cpi.PROYECTO = T_SellosPresupuestales.Proyecto  
		left JOIN C_PartidasPres  ON C_PartidasPres.IdPartida   = T_SellosPresupuestales.IdPartida  
		...
		Declare @TNomPVigente_9_2 as Table(NomPVigente2 decimal(15,2), Clave varchar (max), Clave4 varchar(Max), ID int)
		insert into @TNomPVigente_9_2
		Select  (sum(ISNULL(TP.Autorizado,0)) + (sum(ISNULL(TP.Ampliaciones,0)) + sum(ISNULL(TP.TransferenciaAmp,0))) - (sum(ISNULL(TP.Reducciones,0)) 
		+ sum(ISNULL(TP.TransferenciaRed,0)))) as NomPVigente2, cr.CLAVE, cpi.clave as Clave4, cepr.Id 
		FROM T_SellosPresupuestales INNER JOIN T_PRESUPUESTONW TP ON T_SellosPresupuestales.IdSelloPresupuestal = TP.IdSelloPresupuestal
		INNER JOIN C_RamoPresupuestal  CR ON CR.IDRAMOPRESUPUESTAL = T_SellosPresupuestales.IdRamoPresupuestal  INNER JOIN C_EP_Ramo cepr
		ON cepr.Id    = T_SellosPresupuestales.IdProyecto  INNER JOIN C_ProyectosInversion cpi 	ON cpi.PROYECTO = T_SellosPresupuestales.Proyecto  
		left JOIN C_PartidasPres  ON C_PartidasPres.IdPartida   = T_SellosPresupuestales.IdPartida  left JOIN C_ClasificadorGeograficoPresupuestal cgp 
