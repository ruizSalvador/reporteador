IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_RPT_Relacion_Analitica_Terrenos_ISAF]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_RPT_Relacion_Analitica_Terrenos_ISAF]
GO
...
--EXEC SP_RPT_Relacion_Analitica_Terrenos_ISAF 6,6,2019;
CREATE PROCEDURE [dbo].[SP_RPT_Relacion_Analitica_Terrenos_ISAF]
@MesInicio smallint,    
@MesFin smallint,
@Ejercicio smallint
...
AND SUBSTRING(CuentaAcumulacion,1,LEN('1231')) = '1231' AND Afectable != 0--AND SUBSTRING(CuentaAcumulacion,1,LEN('1112')) = '1112'
GROUP BY Cont.NumeroCuenta, Cont.NombreCuenta, TipoCuenta, CargosSinFlujo, AbonosSinFlujo, TotalCargos, TotalAbonos, TipoCuenta, CargosSinFlujo, AbonosSinFlujo, TotalCargos, TotalAbonos
ORDER BY Cont.NumeroCuenta;

SELECT * FROM @Tabla WHERE (NumeroCuenta = '1231' OR SUBSTRING(NumeroCuenta,1,LEN('1231')) = '1231');

END

GO
EXEC SP_FirmasReporte 'Relacion Analitica de Terrenos ISAF' 

GO

Exec SP_CFG_LogScripts 'SP_RPT_Relacion_Analitica_Terrenos_ISAF','2.29'
GO