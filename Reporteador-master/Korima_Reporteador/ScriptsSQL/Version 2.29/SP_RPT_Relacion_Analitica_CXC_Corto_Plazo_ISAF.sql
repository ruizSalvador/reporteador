IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_RPT_Relacion_Analitica_CXC_Corto_Plazo_ISAF]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_RPT_Relacion_Analitica_CXC_Corto_Plazo_ISAF]
GO
...
--EXEC SP_RPT_Relacion_Analitica_CXC_Corto_Plazo_ISAF 6,6,2019;
CREATE PROCEDURE [dbo].[SP_RPT_Relacion_Analitica_CXC_Corto_Plazo_ISAF]
@MesInicio smallint,    
@MesFin smallint,
@Ejercicio smallint
...
--	SUM(CASE Cont.TipoCuenta WHEN 'A' THEN CargosSinFlujo - AbonosSinFlujo + TotalCargos - TotalAbonos WHEN 'C' THEN CargosSinFlujo - AbonosSinFlujo + TotalCargos - TotalAbonos WHEN 'E' THEN CargosSinFlujo - AbonosSinFlujo
...
--AND SUBSTRING(CuentaAcumulacion,1,LEN('1122')) = '1122' AND Afectable != 0--AND SUBSTRING(CuentaAcumulacion,1,LEN('1112')) = '1112'
...
LEFT JOIN (Select C_Contable.IdCuentaContable, 
			...
			AND SUBSTRING(CuentaAcumulacion,1,LEN('1122')) = '1122' AND Afectable != 0 ) SaldosIniciales
...
LEFT JOIN (Select C_Contable.IdCuentaContable, 
			...
			AND SUBSTRING(CuentaAcumulacion,1,LEN('1122')) = '1122' AND Afectable != 0) SaldosFinales 
...
SELECT * FROM @Tabla WHERE (NumeroCuenta = '1122' OR SUBSTRING(NumeroCuenta,1,LEN('1122')) = '1122');

END

GO
EXEC SP_FirmasReporte 'Relacion Analitica CXC a Corto Plazo ISAF' 

GO

Exec SP_CFG_LogScripts 'SP_RPT_Relacion_Analitica_CXC_Corto_Plazo_ISAF','2.29'
GO