...
--Exec SP_RPT_K2_EgresosClasificacionAdministrativaPoder 1,4,2016,0,1,1
...
UPDATE @Titulos  
SET T.Aprobado=ISNULL(T2.Aprobado,0),
	T.AmpliacionesReducciones= ISNULL(T2.AmpliacionesReducciones,0),
	T.Modificado=ISNULL(T2.Modificado,0),
	T.Devengado=ISNULL(T2.Devengado,0),
	T.Pagado=ISNULL(T2.Pagado,0),
	T.SubEjercicio=ISNULL(T2.SubEjercicio,0)
FROM @Titulos T
JOIN @Valores T2
...
If @AmpRedAnual = 1
	Begin
		update r set r.AmpliacionesReducciones = a.AmpliacionesReducciones FROM @Anual a, @Titulos r Where substring(a.Clave,2,5) =  substring(r.Clave,2,5)
	End 

--SELECT * FROM @Titulos
SELECT Clave, Descripcion, ISNULL(Aprobado,0) as Aprobado,ISNULL(AmpliacionesReducciones,0) as AmpliacionesReducciones, ISNULL(Modificado,0) as Modificado, ISNULL(Devengado,0) as Devengado, ISNULL(Pagado,0) as Pagado, ISNULL(SubEjercicio,0) as SubEjercicio FROM @Titulos
GO
--Clave	Descripcion	Aprobado	AmpliacionesReducciones	Modificado	Devengado	Pagado	SubEjercicio

exec SP_FirmasReporte 'Estado Analítico del Ejercicio del Presupuesto de Egresos Por Poder'
GO
Exec SP_CFG_LogScripts 'SP_RPT_K2_EgresosClasificacionAdministrativaPoder','2.29'
GO
