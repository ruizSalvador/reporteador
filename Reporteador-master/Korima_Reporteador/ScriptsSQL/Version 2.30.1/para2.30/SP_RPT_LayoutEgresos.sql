...
--Exec SP_RPT_LayoutEgresos '20200126',2020,1
...
CASE WHEN @AmpRedAnual = 0 THEN 
	ISNULL((Select SUM(Ampliaciones) from T_PresupuestoNW TP Inner Join T_SellosPresupuestales TS On TP.IdSelloPresupuestal = TS.IdSelloPresupuestal WHERE LYear = @Ejercicio AND (Mes Between 1 and @mes)),0) + ISNULL((Select SUM(TransferenciaAmp) from T_PresupuestoNW TP Inner Join T_SellosPresupuestales TS On TP.IdSelloPresupuestal = TS.IdSelloPresupuestal WHERE LYear = @Ejercicio AND (Mes Between 1 and 12)),0)
ELSE
...
ISNULL((Select SUM(Importe) from T_MovimientosPresupuesto Where IdSelloPresupuestalOrigen = IdSelloPresupuestalDestino and T_MovimientosPresupuesto.Estatus='A' and Ejercidio=@Ejercicio and IdSelloPresupuestalOrigen <>0),0),
...
WHILE @@FETCH_STATUS = 0
	...
	CASE WHEN @AmpRedAnual = 0 THEN 
		ISNULL((Select SUM(Ampliaciones)  from T_PresupuestoNW TP Inner Join T_SellosPresupuestales TS On TP.IdSelloPresupuestal = TS.IdSelloPresupuestal WHERE LYear = @Ejercicio AND (MES Between 1 and @mes) and TP.IdSelloPresupuestal = @IdSello),0) + ISNULL((Select SUM(TransferenciaAmp) from T_PresupuestoNW TP Inner Join T_SellosPresupuestales TS On TP.IdSelloPresupuestal = TS.IdSelloPresupuestal WHERE LYear = @Ejercicio AND (MES Between 1 and 12) and TP.IdSelloPresupuestal = @IdSello),0)
	ELSE
	...
	ISNULL((Select SUM(Importe) from T_MovimientosPresupuesto Where IdSelloPresupuestalOrigen = IdSelloPresupuestalDestino and IdSelloPresupuestalOrigen= @IdSello and T_MovimientosPresupuesto.Estatus='A' and Ejercidio=@Ejercicio and IdSelloPresupuestalOrigen <>0),0),
