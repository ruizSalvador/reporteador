...
--	where T_AltaActivos.Importe>0 and T_Activos.FechaUA >=@dateini and T_Activos.FechaUA <=@datefin and T_Activos.Estatus <> 'B'
	...
	Insert @TablaTodos 
	Select 0 As Total, contable.NumeroCuenta, contable.nombreCuenta, isnull(@totalBienes,0)as TotalBienes, null as Fin, 0 as Diferencia
		From C_Contable contable  
		Where  TipoCuenta <> 'X' 
	  ...
	  Update @TablaTodos set Total = (Select isnull(SUM(saldos.TotalCargos)- SUM(saldos.TotalAbonos),0) From C_Contable contable INNER JOIN T_SaldosInicialesCont saldos ON contable.IdCuentaContable = saldos.IdCuentaContable Where  TipoCuenta <> 'X' and contable.NumeroCuenta = '12360-00000' and saldos.Year = @EjercicioFin and saldos.Mes Between @mesInicio and @mesFin) where NumeroCuenta= '12360-00000'
  	  Update @TablaTodos set Fin = (Select top 1 Fin from @tabla1)

	 
If @Depreciacion = 1
	 Begin
		Update @TablaTodos set Diferencia = ((Select top 1 Fin from @tabla1) - ((Select Total from @TablaTodos where NumeroCuenta= '12350-00000')+(Select Total from @TablaTodos  where NumeroCuenta= '12360-00000')))
	 End
  Else
  Begin 
	Update @TablaTodos set Diferencia = (Select top 1 Fin from @tabla1)
  End		
		Select *
		from @TablaTodos 
...
Else
 ...
 End
 GO
  --EXEC SP_RPT_K2_RelacionBienesPatrimonio_CuentasPeriodo 1,7,1900,2016,0,0
