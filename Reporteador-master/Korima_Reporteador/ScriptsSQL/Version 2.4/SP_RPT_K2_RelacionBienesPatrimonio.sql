...
@Ejercicio int 
AS
Declare @tabla1 as table (NumeroEconomico int, Descripcion varchar(max),Valor decimal(15,2))
Declare @tabla2 as table (NumeroEconomico int, Descripcion varchar(max),Valor decimal(15,2))

insert @tabla1 
select T_Activos.NumeroEconomico,
...
FROM @Tabla1 T JOIN @tabla2 T2 ON T.NumeroEconomico=T2.NumeroEconomico

Select * from @tabla1 order by NumeroEconomico 
