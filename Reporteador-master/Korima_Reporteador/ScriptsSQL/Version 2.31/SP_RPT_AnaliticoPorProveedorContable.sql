...
Select Distinct TP.Folio as OSOC,
		...
		END as Estatus,
		(Select CAST(T_Polizas.TipoPoliza as varchar(10)) + ' '  + CAST(T_Polizas.Periodo as varchar (5)) + ' ' + CAST(T_Polizas.NoPoliza as varchar (50)) from T_Polizas where T_Polizas.IdPoliza = TP.IdPoliza) as PolizaComprometido,
		(Select Fecha  from T_Polizas where T_Polizas.IdPoliza = TP.IdPoliza) as FechaPolizaComprometido, 
...
where TP.Fecha >= @FechaInicio and TP.Fecha <= @FechaFin
	...
	--AND TC.Status <> 'L' 
Group by TC.IdChequesAgrupador, TP.Folio, CP.RazonSocial, TP.TotalGral, TP.Estatus, TRF.Serie, TRF.Factura, TRF.IdPoliza, TP.IdPoliza,
...
Insert into @OS
 Select Distinct TOS.Folio as OSOC,
		...
		WHEN 'A' THEN 'Aprobado'
		END as Estatus,
		(Select CAST(T_Polizas.TipoPoliza as varchar(10)) + ' '  + CAST(T_Polizas.Periodo as varchar (5)) + ' ' + CAST(T_Polizas.NoPoliza as varchar (50)) from T_Polizas where T_Polizas.IdPoliza = TOS.IdPoliza) as PolizaComprometido,
		(Select Fecha from T_Polizas where T_Polizas.IdPoliza = TOS.IdPoliza) as FechaPolizaComprometido,
	    TRF.Serie+' '+TRF.Factura as Factura,
		(Select CAST(T_Polizas.TipoPoliza as varchar(10)) + ' '  + CAST(T_Polizas.Periodo as varchar (5)) + ' ' + CAST(T_Polizas.NoPoliza as varchar (50)) from T_Polizas where T_Polizas.IdPoliza = TRF.IdPoliza) as PolizaDevengado,
		(Select Fecha from T_Polizas where T_Polizas.IdPoliza = TRF.IdPoliza) as FechaPolizaDevengado,
	    --(Select CAST(T_Polizas.TipoPoliza as varchar(10)) + ' '  +CAST(T_Polizas.Periodo as varchar (5)) + ' ' + CAST(T_Polizas.NoPoliza as varchar (50)) from T_Polizas where T_Polizas.IdCheque= TCH.IdCheques ) as PolizaAnticipo,
...
where TOS.Fecha >= @FechaInicio and TOS.Fecha <= @FechaFin
	...
	--AND TC.Status <> 'L'
Group by TC.IdChequesAgrupador, TOS.Folio, CP.RazonSocial, TOS.TotalGral, TOS.Estatus, TRF.Serie, TRF.Factura, TRF.IdPoliza, TOS.IdPoliza,
TSC.Fecha, TSC.FolioPorTipo, TSC.IdPolizaPresupuestoEjercido, TSC.FolioDesconcentrado, TC.FolioCheque, TC.Status, TC.IdCheques,
