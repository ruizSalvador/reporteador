...
--  Where T_AltaActivos.Importe>0 and T_AltaActivos.FechaAlta >=@dateini and T_AltaActivos.FechaAlta<=@datefin 
 ...
 --   Select top 1 T_Depreciaciones.IdDepreciacion from t_Depreciaciones Where Ano = @EjercicioFin and Mes = @mesFin order by IdDepreciacion desc)
	...
	--Update t set Valor = 
	--CASE Valor When 0 THEN (Select CostoContable from T_Activos where NumeroEconomico = t.NumeroEconomico)
	--Else Valor END
	--from @tabla2 t

	--EXEC SP_RPT_K2_RelacionBienesPatrimonioPeriodo 1,3,1900,2021,0,1
	--Select @dateini
	--Select @datefin
	--Select * from @tabla1
	--Select * from @tabla2
...
GO
Exec SP_CFG_LogScripts 'SP_RPT_K2_RelacionBienesPatrimonioPeriodo','2.30.1'
GO
 --EXEC SP_RPT_K2_RelacionBienesPatrimonioPeriodo 1,2,1937,2021,0,1
