...
-- Exec SP_ControlPresupuestalIngreso 7,2020,0
...
Insert into @RubroMes
Exec SP_EstadoEjercicioIngresos_Rubro @MesFin,@MesFin,0,@Ejercicio,0,@RedAmpAcum

--Select * from @RubroMes
...
FROM         @PresupuestoFlujo MovimientosPresupuesto  LEFT OUTER JOIN
					  ...
					  WHERE MovimientosPresupuesto.Mes Between 1 and 12
...
UPDATE A SET A.EstimadoMes = B.Total_Estimado, A.RecaudadoMes = B.Total_Recaudado FROM @Resultado A INNER JOIN @RubroMes B ON A.Clave = B.Clave and A.Clave like '_0000'
UPDATE A SET A.EstimadoMes = B.Total_Estimado, A.RecaudadoMes = B.Total_Recaudado FROM @Resultado A INNER JOIN @RubroMes B ON A.Clasificacion = B.Clasificacion and A.Clasificacion = 'Total'
...
GO
Exec SP_CFG_LogScripts 'SP_ControlPresupuestalIngreso','2.30'
GO
