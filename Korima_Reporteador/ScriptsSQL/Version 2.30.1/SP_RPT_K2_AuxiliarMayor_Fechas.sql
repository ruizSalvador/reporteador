...
=======================================================================================================  
*/      
 /****** Object:  StoredProcedure [dbo].[SP_RPT_K2_AuxiliarMayor_Fechas] ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_RPT_K2_AuxiliarMayor_Fechas]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_RPT_K2_AuxiliarMayor_Fechas]
GO
/****** Object:  StoredProcedure [dbo].[SP_RPT_K2_AuxiliarMayor_Fechas]   ******/
SET ANSI_NULLS ON
GO
...
set @iCuentaFin= CONVERT(bigint,REPLACE(@CuentaFin,'-',''))    
 ...
 IF (@NumeroCuenta = '' and @CuentaAcumulable <> '') OR @CuentaInicio =''
 BEGIN   
  --Select '1'
 INSERT into @tabla      
  SELECT    
...
FROM T_Polizas    
JOIN    
D_Polizas ON D_Polizas.IdPoliza= T_Polizas.IdPoliza AND D_Polizas.IdPoliza in (Select IdPoliza from T_Polizas Where Ejercicio = YEAR(@FechaFin))
JOIN    
...
T_Cheques ON T_Cheques.IdCheques=T_Polizas.IdCheque     
 ...
 and( T_Cheques.Status= 'D' or ((T_Cheques.Status='I'OR T_Cheques.Status='N')))and T_Polizas.NoPoliza>0))    
  ...
  AND [year] = YEAR(@FechaFin)
  AND T_Polizas.Ejercicio =  YEAR(@FechaFin)    
...
BEGIN    
  ...
  Fecha datetime    
   ...
   ,Afectable int   
	...
	BEGIN 
		...
		0 as SaldoInicial,
		0 as SaldoFinal,    
		MONTH(@FechaIni) as Mes,    
		YEAR(@FechaIni) as Year,    
		C_Contable.TipoCuenta,     
		Null as Numero,    
		Null as FolioCheque,    
		null as ConceptoPago,    
		...
		AND VW_C_Contable.iNumeroCuenta BETWEEN @iCuentaInicio and @iCuentaFin 
		and C_Contable.NumeroCuenta not in (Select NumeroCuentaContable from @tablaVacios where Year =YEAR(@FechaFin) and (Mes between MONTH(@FechaIni) and MONTH(@FechaFin)))
		END
		ELSE
		BEGIN
		--Vacios NO existentes en el periodo    
		...
		Afectable    
		from C_Contable    
		where Nivel>=0 and C_Contable.NumeroCuenta not in (Select NumeroCuentaContable from @tablaVacios where Year =YEAR(@FechaFin) and (Mes between MONTH(@FechaIni) and MONTH(@FechaFin)))
		END   
  ...
  AND Mes >= MONTH(@FechaIni)    
  AND Mes <= MONTH(@FechaFin)     
  And year = YEAR(@FechaFin)    
  and NumeroCuentaContable not in (select NumeroCuentaContable  from @tabla where year=YEAR(@FechaFin) and(Mes between MONTH(@FechaIni) and MONTH(@FechaFin) ))    
  END  
  ...
  insert @tabla2     
    ...
    Fecha    
     ...
     ,Afectable    
  	...
  	,''
  	,''   
     from @tabla where  (Mes between MONTH(@FechaIni) AND  MONTH(@FechaFin)) And year = YEAR(@FechaFin) 
