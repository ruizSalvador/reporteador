...
T_RecepcionFacturas.Total as Facturado,
(Select SUM(Importe) from D_RecepcionFacturas Where IdRecepcionServicios = T_RecepcionFacturas.IdRecepcionServicios) as Pagado,
T_RecepcionFacturas.Total - (Select SUM(Importe) from D_RecepcionFacturas Where IdRecepcionServicios = T_RecepcionFacturas.IdRecepcionServicios) as Pendiente,
...
WHEN (T_RecepcionFacturas.Total - (Select SUM(Importe) from D_RecepcionFacturas Where IdRecepcionServicios = T_RecepcionFacturas.IdRecepcionServicios)) <> 0 THEN 'PAGADA PARCIALMENTE'
WHEN (T_RecepcionFacturas.Total - (Select SUM(Importe) from D_RecepcionFacturas Where IdRecepcionServicios = T_RecepcionFacturas.IdRecepcionServicios)) = 0 THEN 'PAGADA TOTALMENTE'
END AS Estatus,
