...
/****** Object:  StoredProcedure [dbo].[SP_RPT_K2_RelacionBienesPatrimonio_CuentasPeriodoFiltrado]    Script Date: 10/25/2013 17:14:38 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_RPT_K2_RelacionBienesPatrimonio_CuentasPeriodoFiltrado]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_RPT_K2_RelacionBienesPatrimonio_CuentasPeriodoFiltrado]
GO
...
--EXEC SP_RPT_K2_RelacionBienesPatrimonio_CuentasPeriodoFiltrado 1,10,2016,2016,0,0,0
CREATE PROCEDURE [dbo].[SP_RPT_K2_RelacionBienesPatrimonio_CuentasPeriodoFiltrado]
@mesInicio int,
@mesFin int,
@EjercicioInicio int,
@EjercicioFin int,
...
if @General = 0
  Begin	
	...
	JOIN C_TipoBien ON T_Activos.IdTipoBien = C_TipoBien.IdTipoBien 
--	where T_AltaActivos.Importe>0 and T_Activos.FechaUA >=@dateini and T_Activos.FechaUA <=@datefin and T_Activos.Estatus <> 'B'
    where C_TipoBien.IdCuentaContable = CASE WHEN @IdTipoBien = 0 THEN C_TipoBien.IdCuentaContable ELSE  @IdTipoBien END
	and T_AltaActivos.Importe>0 and T_AltaActivos.FechaAlta >=@dateini and T_AltaActivos.FechaAlta<=@datefin 
	...
	IF @Depreciacion = 1
		 begin
			...
			--Update @tabla1 set Fin = ISNULL((Select SaldoDeudor from @Resultado where CuentaNumero = 1200000000),0)
			 update @tabla1 set Fin = ISNULL((SELECT SUM(Valor) from @tabla1),0)
			-- Select * from @tabla1
		 end
	 ELSE
		 begin
	...
	Select 0 As Total, contable.NumeroCuenta, contable.nombreCuenta, isnull(@totalBienes,0)as TotalBienes, 0 as Fin, 0 as Diferencia
		...
		or NumeroCuenta = '123500-000000' or NumeroCuenta = '123600-000000' 
		or NumeroCuenta = '123500-00000' or NumeroCuenta = '123600-00000'
		GROUP BY contable.NumeroCuenta, contable.nombreCuenta
...
If @Depreciacion = 1
	 Begin
		Update @TablaTodos set Diferencia = ISNULL(((Select top 1 Fin from @tabla1) - ((Select Total from @TablaTodos where NumeroCuenta= '12350-00000' or NumeroCuenta= '123500-000000' or NumeroCuenta = '123500-00000' )+(Select Total from @TablaTodos  where NumeroCuenta= '12360-00000' or NumeroCuenta = '123600-000000' or NumeroCuenta = '123600-00000'))),0)
...
Else
 ...
 EXEC SP_CFG_LogScripts 'SP_RPT_K2_RelacionBienesPatrimonio_CuentasPeriodoFiltrado'
GO
