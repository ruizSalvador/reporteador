...
--  where T_AltaActivos.Importe>0 and T_AltaActivos.FechaAlta >=@dateini and T_AltaActivos.FechaAlta<=@datefin 
	...
	group by d_depreciaciones.NumeroEconomico
	--------------------------------------------------
 ...
 --   Select top 1 T_Depreciaciones.IdDepreciacion from t_Depreciaciones order by IdDepreciacion desc)
	...
	Select 0 As Total, contable.NumeroCuenta, contable.nombreCuenta, isnull(@totalBienes,0)as TotalBienes, 0 as Fin, 0 as Diferencia
	  ...
	  --Update @TablaTodos set Total = (Select isnull(SUM(saldos.CargosSinFlujo)-SUM(saldos.AbonosSinFlujo) + SUM(saldos.TotalCargos)- SUM(saldos.TotalAbonos),0) From C_Contable contable INNER JOIN T_SaldosInicialesCont saldos ON contable.IdCuentaContable = saldos.IdCuentaContable Where  TipoCuenta <> 'X' and contable.NumeroCuenta = '12350-00000' and saldos.Year = @EjercicioFin and saldos.Mes Between @mesInicio and @mesFin) where NumeroCuenta= '12350-00000'
	  --Update @TablaTodos set Total = (Select isnull(SUM(saldos.CargosSinFlujo)-SUM(saldos.AbonosSinFlujo) + SUM(saldos.TotalCargos)- SUM(saldos.TotalAbonos),0) From C_Contable contable INNER JOIN T_SaldosInicialesCont saldos ON contable.IdCuentaContable = saldos.IdCuentaContable Where  TipoCuenta <> 'X' and contable.NumeroCuenta = '12360-00000' and saldos.Year = @EjercicioFin and saldos.Mes Between @mesInicio and @mesFin) where NumeroCuenta= '12360-00000'

	   Update @TablaTodos set Total = (Select ISNULL(SaldoDeudor,0) - ISNULL(SaldoAcreedor,0)  From VW_RPT_K2_Balanza_De_Comprobacion Where NumeroCuenta = '12350-00000' and Year = @EjercicioFin and Mes = @mesBal) where NumeroCuenta= '12350-00000'
	  Update @TablaTodos set Total = (Select ISNULL(SaldoDeudor,0) - ISNULL(SaldoAcreedor,0)  From VW_RPT_K2_Balanza_De_Comprobacion Where NumeroCuenta = '12360-00000' and Year = @EjercicioFin and Mes = @mesBal) where NumeroCuenta= '12360-00000'
...
If @Depreciacion = 1
  ...
  End		
		Select ISNULL(Total,0) as Total,NumeroCuenta,NombreCuenta,TotalBienes,Fin,Diferencia
		from @TablaTodos 
...
Else
 Begin
	...
	Select 0 As Total, contable.NumeroCuenta, contable.nombreCuenta, isnull(@totalBienes,0) as TotalBienes, 0 as Fin, 0 as Diferencia
	  ...
	  --Update @TablaTodos set Total = (Select isnull(SUM(saldos.CargosSinFlujo)-SUM(saldos.AbonosSinFlujo) + SUM(saldos.TotalCargos)- SUM(saldos.TotalAbonos),0) From C_Contable contable INNER JOIN T_SaldosInicialesCont saldos ON contable.IdCuentaContable = saldos.IdCuentaContable Where  TipoCuenta <> 'X' and contable.NumeroCuenta = '12350-00000' ) where NumeroCuenta= '12350-00000'
	  --Update @TablaTodos set Total = (Select isnull(SUM(saldos.CargosSinFlujo)-SUM(saldos.AbonosSinFlujo) + SUM(saldos.TotalCargos)- SUM(saldos.TotalAbonos),0) From C_Contable contable INNER JOIN T_SaldosInicialesCont saldos ON contable.IdCuentaContable = saldos.IdCuentaContable Where  TipoCuenta <> 'X' and contable.NumeroCuenta = '12360-00000' ) where NumeroCuenta= '12360-00000'
	  --Update @TablaTodos set Fin = ISNULL((Select top 1 Fin from @tabla1),0)
 ...
 Exec SP_CFG_LogScripts 'SP_RPT_K2_RelacionBienesPatrimonio_CuentasPeriodo','2.31'
 GO
  --EXEC SP_RPT_K2_RelacionBienesPatrimonio_CuentasPeriodo 1,7,2021,2021,0,1
