...
DECLARE @eAnterior TABLE (
	...
	SaldoAcreedor decimal(15,2))
--Tabla del Ejercicio Anterior
DECLARE @eAnteriorPeriodo TABLE (
	nocuenta varchar (30), 
	nombre varchar(255), 
	cSinFlujo decimal(15,2), 
	aSinFlujo decimal(15,2),
...
From C_Contable LEFT JOIN 
(Select IdCuentaContable, Year,Mes,SUM(isnull(TotalCargos,0)) as TotalCargos,SUM(isnull(TotalAbonos,0))as TotalAbonos,SUM(isnull(CargosSinFlujo,0)) as CargosSinFlujo,SUM(isnull(AbonosSinFlujo,0)) as AbonosSinFlujo 
From T_SaldosInicialesCont Where ( Mes between @PeriodoFin and @PeriodoFin) And [Year] = @ejercicio group by IdCuentaContable,Year,Mes) Saldos
On C_Contable.IdCuentaContable = Saldos.IdCuentaContable
Where TipoCuenta <> 'X' 
Group By NumeroCuenta,NombreCuenta, TipoCuenta
Order By NumeroCuenta
...
From C_Contable LEFT JOIN 
(Select IdCuentaContable, Year,Mes,SUM(isnull(TotalCargos,0)) as TotalCargos,SUM(isnull(TotalAbonos,0))as TotalAbonos,SUM(isnull(CargosSinFlujo,0)) as CargosSinFlujo,SUM(isnull(AbonosSinFlujo,0)) as AbonosSinFlujo 
From T_SaldosInicialesCont Where ( Mes between @PeriodoFin and @PeriodoFin) And [Year] = @ejercicio-1 group by IdCuentaContable,Year,Mes) Saldos
On C_Contable.IdCuentaContable = Saldos.IdCuentaContable
Where TipoCuenta <> 'X' 
Group By NumeroCuenta,NombreCuenta,TipoCuenta
Order By NumeroCuenta
...
--CAMBIOS PARA 5-6 INICIO /DM 20180707
...
/*
(select(select ISNULL(sum(importecargo),0) from d_polizas where idpoliza in ( select idpoliza from t_polizas where tipopoliza in('I','D','E') and MONTH(Fecha) Between @periodo and @periodoFin and Ejercicio = @ejercicio and idtipomovimiento <> 404  and nopoliza <> 0) and idcuentacontable in (select idcuentacontable from c_contable where numerocuenta like '111%')and ImporteCargo <> 0)
-(SELECT isnull(SUM(IMPORTEABONO),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @EJERCICIO) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '81500-01%'))
-(SELECT isnull(SUM(IMPORTEABONO),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @EJERCICIO) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '81500-02%'))
-(SELECT isnull(SUM(IMPORTEABONO),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @EJERCICIO) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '81500-03%'))
-(SELECT isnull(SUM(IMPORTEABONO),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @EJERCICIO) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '81500-04%'))
-(SELECT  isnull(SUM(IMPORTEABONO),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @ejercicio) 
AND IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '81500-05%'))
-(SELECT isnull(SUM(IMPORTEABONO),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @EJERCICIO) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '81500-06%'))
-(SELECT isnull(SUM(IMPORTEABONO),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @EJERCICIO) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '81500-07%'))
-(SELECT isnull(SUM(IMPORTEABONO),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @EJERCICIO) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '81500-08%'))
-(SELECT isnull(SUM(IMPORTEABONO),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @EJERCICIO) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '81500-09%'))
+(select isnull(sum(Importeabono),0) from d_polizas where idpoliza in (select idpoliza from T_Polizas where IdTipoMovimiento = 404 and ejercicio = @EJERCICIO and Periodo = @periodo) and IdCuentaContable = 3866)),
--año anterior
(select(select ISNULL(sum(importecargo),0) from d_polizas where idpoliza in ( select idpoliza from t_polizas where tipopoliza in('I','D','E') and MONTH(Fecha) Between @periodo and @periodoFin and Ejercicio = @ejercicio-1 and idtipomovimiento <> 404  and idpolizacancelacion is null) and idcuentacontable in (select idcuentacontable from c_contable where numerocuenta like '111%')and ImporteCargo <> 0)
-(SELECT isnull(SUM(IMPORTEABONO),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @EJERCICIO-1) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '81500-01%'))
-(SELECT isnull(SUM(IMPORTEABONO),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @EJERCICIO-1) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '81500-02%'))
-(SELECT isnull(SUM(IMPORTEABONO),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @EJERCICIO-1) AND 
...
-(SELECT isnull(SUM(IMPORTEABONO),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @ejercicio-1) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '81500-04%'))
-(SELECT  isnull(SUM(IMPORTEABONO),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @ejercicio-1) 
AND IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '81500-05%'))
-(SELECT isnull(SUM(IMPORTEABONO),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @EJERCICIO-1) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '81500-06%'))
-(SELECT isnull(SUM(IMPORTEABONO),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @EJERCICIO-1) AND 
...
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '81500-08%'))
-(SELECT isnull(SUM(IMPORTEABONO),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @EJERCICIO-1) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '81500-09%'))),0, 'X',19
*/
(select(select ISNULL(sum(importecargo),0) from d_polizas where idpoliza in ( select idpoliza from t_polizas where tipopoliza in('I','D','E') and MONTH(Fecha) Between @periodo and @periodoFin and Ejercicio = @ejercicio and idtipomovimiento <> 404  and nopoliza <> 0) and idcuentacontable in (select idcuentacontable from c_contable where numerocuenta like '111%')and ImporteCargo <> 0)
-(SELECT isnull(SUM(IMPORTEABONO),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @EJERCICIO) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '81500-1%'))
-(SELECT isnull(SUM(IMPORTEABONO),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @EJERCICIO) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '81500-2%'))
-(SELECT isnull(SUM(IMPORTEABONO),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @EJERCICIO) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '81500-3%'))
-(SELECT isnull(SUM(IMPORTEABONO),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @EJERCICIO) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '81500-4%'))
-(SELECT  isnull(SUM(IMPORTEABONO),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @ejercicio) 
AND IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '81500-5%'))
-(SELECT isnull(SUM(IMPORTEABONO),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @EJERCICIO) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '81500-6%'))
-(SELECT isnull(SUM(IMPORTEABONO),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @EJERCICIO) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '81500-7%'))
-(SELECT isnull(SUM(IMPORTEABONO),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @EJERCICIO) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '81500-8%'))
-(SELECT isnull(SUM(IMPORTEABONO),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @EJERCICIO) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '81500-9%'))
+(select isnull(sum(Importeabono),0) from d_polizas where idpoliza in (select idpoliza from T_Polizas where IdTipoMovimiento = 404 and ejercicio = @EJERCICIO and Periodo = @periodo) and IdCuentaContable = 3866)),
--año anterior
(select(select ISNULL(sum(importecargo),0) from d_polizas where idpoliza in ( select idpoliza from t_polizas where tipopoliza in('I','D','E') and MONTH(Fecha) Between @periodo and @periodoFin and Ejercicio = @ejercicio-1 and idtipomovimiento <> 404  and idpolizacancelacion is null) and idcuentacontable in (select idcuentacontable from c_contable where numerocuenta like '111%')and ImporteCargo <> 0)
-(SELECT isnull(SUM(IMPORTEABONO),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @EJERCICIO-1) AND 
...
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '81500-8%'))
-(SELECT isnull(SUM(IMPORTEABONO),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @EJERCICIO-1) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '81500-9%'))),0, 'X',19
-- A 4
Union All
Select '',null,null,3,'X',19
...
where IdTipoMovimiento in (415,140,143,408) and YEAR(Fecha) = @ejercicio and (MONTH(Fecha) Between @periodo and @periodoFin and status <> 'N' and IdChequesAgrupador =0 and status <> 'C' and status <> 'G')),
(Select SUM(ImporteCheque) from T_Cheques 
where IdTipoMovimiento in (415,140,143,408) and YEAR(Fecha) = @ejercicio-1 and (MONTH(Fecha) Between @periodo and @periodoFin and status <> 'N' and IdChequesAgrupador =0 and status <> 'C' and status <> 'G')), 
0, 'X', 21
--Capitulo 2000
UNION ALL
Select 'MATERIALES Y SUMINISTROS',
(select(Select SUM(ImporteCheque) from T_Cheques 
...
-(Select isnull(SUM(importecargo),0) From d_polizas Where idpoliza in (select idpoliza from t_polizas where IdTipoMovimiento in (7) and YEAR(Fecha) = @ejercicio and MONTH(Fecha) Between @periodo and @periodoFin  and tipopoliza = 'D' and idpoliza <> 580 )and IdCuentaContable in (select idcuentacontable from c_Contable where numerocuenta like '82700-05%'))
-(select ISNULL(sum(importecargo),0) from d_polizas where idpoliza in (select idpoliza from t_polizas where MONTH(Fecha)Between @periodo and @periodoFin and YEAR(Fecha) = @ejercicio and IdTipoMovimiento = 146) and idcuentacontable in ( select idcuentacontable from  c_contable where numerocuenta like '82700-05%')
)),
(Select SUM(ImporteCheque) from T_Cheques 
...
(select(Select isnull(SUM(ImporteCheque),0) From T_Cheques 
Where IdTipoMovimiento in (235,163) and YEAR(Fecha) = @ejercicio and MONTH(Fecha) Between @periodo and @periodoFin and idCHEQUESAGRUPADOR = 0 and status <> 'N' and status <> 'C' and status <> 'G')
+(Select isnull(SUM(ImporteCheque),0) From T_Cheques Where IdTipoMovimiento in (235,163) and YEAR(Fecha) = @ejercicio and MONTH(Fecha) Between @periodo and @periodoFin and idCHEQUESAGRUPADOR = 0 and month(fecha) <> month(fechacancelacion) and status <> 'C' and status <> 'G')
-(select ISNULL(sum(importecargo),0) from d_polizas where idpoliza in (select idpoliza from t_polizas where MONTH(Fecha)Between @periodo and @periodoFin and YEAR(Fecha) = @ejercicio and IdTipoMovimiento in (235,163)) and idcuentacontable in ( select idcuentacontable from  c_contable where numerocuenta like '82700-05%'))),
(Select isnull(SUM(ImporteCheque),0) From T_Cheques Where IdTipoMovimiento in (235,163) and YEAR(Fecha) = @ejercicio-1 and MONTH(Fecha) Between @periodo and @periodoFin and idCHEQUESAGRUPADOR = 0 and status <> 'N' and status <> 'C' and status <> 'G'),
0, 'X', 23
UNION ALL
Select 'TRANSFERENCIAS INTERNAS Y ASIGNACIONES AL SECTOR PÚBLICO',
(SELECT isnull(SUM(ImporteCargo),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @ejercicio) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '82700-41%')),
(SELECT isnull(SUM(ImporteCargo),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @ejercicio-1) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '82700-41%')),0, 'X', 25
UNION ALL
Select 'TRANSFERENCIAS AL RESTO DEL SECTOR PÚBLICO',
(SELECT isnull(SUM(ImporteCargo),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @ejercicio) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '82700-42%')),
(SELECT isnull(SUM(ImporteCargo),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @ejercicio-1) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '82700-42%')), 
0, 'X', 26
UNION ALL
Select 'SUBSIDIOS Y SUBVENCIONES',
(SELECT isnull(SUM(ImporteCargo),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @ejercicio and nopoliza <> 0) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '82700-43%')),
(SELECT isnull(SUM(ImporteCargo),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @ejercicio-1 and nopoliza <> 0) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '82700-43%')), 
0, 'X', 27
UNION ALL
Select 'AYUDAS SOCIALES',
(SELECT isnull(SUM(ImporteCargo),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @ejercicio and nopoliza <> 0) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '82700-44%')and iddpoliza <>109815 and iddpoliza <>109823),
(SELECT isnull(SUM(ImporteCargo),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @ejercicio-1 and nopoliza <> 0) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '82700-44%')and iddpoliza <>109815 and iddpoliza <>109823), 0, 'X', 28
UNION ALL
Select 'PENSIONES Y JUBILACIONES',
(SELECT isnull(SUM(ImporteCargo),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @ejercicio and nopoliza <> 0) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '82700-45%')),
(SELECT isnull(SUM(ImporteCargo),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @ejercicio-1 and nopoliza <> 0) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '82700-45%')), 
0, 'X', 29
UNION ALL
...
(SELECT isnull(SUM(ImporteCargo),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @ejercicio and nopoliza <> 0) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '82700-46%')),
(SELECT isnull(SUM(ImporteCargo),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @ejercicio-1 and nopoliza <> 0) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '82700-46%')), 
0, 'X', 30
UNION ALL
...
(SELECT isnull(SUM(ImporteCargo),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @ejercicio and nopoliza <> 0) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '82700-471%')),
(SELECT isnull(SUM(ImporteCargo),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @ejercicio-1 and nopoliza <> 0) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '82700-471%')), 
0, 'X', 31
UNION ALL
...
(select(select ISNULL(sum(importeabono),0) from d_polizas where idpoliza in ( select idpoliza from t_polizas where tipopoliza in('I','D','E') and MONTH(Fecha) Between @periodo and @periodoFin and Ejercicio = @EJERCICIO and idtipomovimiento <> 404 and NoPoliza <> 0 ) and idcuentacontable in (select idcuentacontable from c_contable where numerocuenta like '111%')and ImporteAbono <> 0)
-(Select isnull(SUM(ImporteCheque),0) from T_Cheques where IdTipoMovimiento in (415,140,143,408) and YEAR(Fecha) = @ejercicio and MONTH(Fecha) Between @periodo and @periodoFin and status <> 'N' and status <> 'C' and IdChequesAgrupador =0 and status <> 'G')
-(Select isnull(SUM(ImporteCheque),0) from T_Cheques where IdTipoMovimiento = 146 and YEAR(Fecha) = @ejercicio and MONTH(Fecha) Between @periodo and @periodoFin and idCHEQUESAGRUPADOR = 0 and status <> 'N' and status <> 'C')
-(Select isnull(SUM(ImporteCheque),0) From T_Cheques Where IdTipoMovimiento in (235,163) and YEAR(Fecha) = @ejercicio and MONTH(Fecha) Between @periodo and @periodoFin and idCHEQUESAGRUPADOR = 0 and status <> 'N' and status <> 'C' and status <> 'G')
-(Select isnull(SUM(ImporteCheque),0) From T_Cheques Where IdTipoMovimiento in (235,163) and YEAR(Fecha) = @ejercicio and MONTH(Fecha) Between @periodo and @periodoFin and idCHEQUESAGRUPADOR = 0 and month(fecha) <> month(fechacancelacion) and status <> 'C' and status <> 'G')
-(SELECT isnull (SUM(IMPORTEcargo),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @ejercicio and nopoliza <> 0) AND IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '82700-4%'))
-(SELECT isnull (SUM(IMPORTEcargo),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @ejercicio and nopoliza <> 0) AND IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '82700-5%'))
-(SELECT isnull (SUM(IMPORTEcargo),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @ejercicio and nopoliza <> 0) AND IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '82700-6%'))
+(select isnull (sum(ImporteCargo),0) from d_polizas where idpoliza in (select idpoliza from T_Polizas where IdTipoMovimiento = 404 and ejercicio = @ejercicio and Periodo = @periodo) and IdCuentaContable = 1501)),
--año anterior--
(select(select ISNULL(sum(importeabono),0) from d_polizas where idpoliza in ( select idpoliza from t_polizas where tipopoliza in('I','D','E') and MONTH(Fecha) Between @periodo and @periodoFin and Ejercicio = @EJERCICIO-1 and idtipomovimiento <> 404 and NoPoliza <> 0  ) and idcuentacontable in (select idcuentacontable from c_contable where numerocuenta like '111%')and ImporteAbono <> 0)
-(Select isnull(SUM(ImporteCheque),0) from T_Cheques where IdTipoMovimiento in (415,140,143,408) and YEAR(Fecha) = @ejercicio-1 and MONTH(Fecha) Between @periodo and @periodoFin and status <> 'N' and status <> 'C' and IdChequesAgrupador =0 and status <> 'G')
-(Select isnull(SUM(ImporteCheque),0) from T_Cheques where IdTipoMovimiento = 146 and YEAR(Fecha) = @ejercicio-1 and MONTH(Fecha) Between @periodo and @periodoFin and idCHEQUESAGRUPADOR = 0 and status <> 'N' and status <> 'C')
-(Select isnull(SUM(ImporteCheque),0) From T_Cheques Where IdTipoMovimiento in (235,163) and YEAR(Fecha) = @ejercicio-1 and MONTH(Fecha) Between @periodo and @periodoFin and idCHEQUESAGRUPADOR = 0 and status <> 'N' and status <> 'C' and status <> 'G')
-(SELECT isnull(SUM(ImporteCargo),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @ejercicio-1) AND IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '82700-44%')and iddpoliza <>109815 and iddpoliza <>109823)
-(Select isnull(SUM(ImporteCheque),0) From T_Cheques Where IdTipoMovimiento in (235,163) and YEAR(Fecha) = @ejercicio-1 and MONTH(Fecha) Between @periodo and @periodoFin and idCHEQUESAGRUPADOR = 0 and fecha <> fechacancelacion and status <> 'C' and status <> 'G')
--(SELECT isnull (SUM(IMPORTEcargo),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodoFin AND Ejercicio = @ejercicio-1 and nopoliza <> 0) AND IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '82700-05%'))
-(SELECT isnull (SUM(IMPORTEcargo),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between 1 and 1 AND Ejercicio = 2017 and nopoliza <> 0 and TipoPoliza = 'E') AND IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '82700-06%'))),
0, 'X', 37.1
insert into @report 
...
Select 'BIENES MUEBLES',
(SELECT isnull(SUM(IMPORTEcargo),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE (MONTH(Fecha) Between @periodo and @periodoFin)  AND Ejercicio = @ejercicio and nopoliza <> 0 ) AND IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '82700-5%')),
(SELECT SUM(IMPORTEcargo) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE (MONTH(Fecha) Between @periodo and @periodoFin)  AND Ejercicio = @ejercicio-1 and nopoliza <> 0) AND IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '82700-5%'))
, 0, 'X', 45
Union All
Select 'BIENES INMUEBLES, INFRAESTRUCTURA Y CONTRUCCIONES EN PROCESO',
(Select  isnull (SUM(TotalCargos),0) from @eActual
Where SUBSTRING(nocuenta ,1,4) In ('8270') And
SUBSTRING(nocuenta ,CHARINDEX('-', nocuenta) + 1 ,3) In ('611','612','613','614','615','616','617','618','619','620','621', '622','623','624','625','626','627','628','629','630','631'))
-(SELECT isnull (SUM(IMPORTEcargo),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE MONTH(Fecha) Between @periodo and @periodofin AND Ejercicio = @ejercicio and nopoliza <> 0 and TipoPoliza = 'D') AND IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '82700-06%')),
(Select SUM(totalcargos) from @eAnterior
Where SUBSTRING(nocuenta ,1,4) In ('8270') And
SUBSTRING(nocuenta ,CHARINDEX('-', nocuenta) + 1 ,3) In ('611','612','613','614','615','616','617','618','619','620','621', '622','623','624','625','626','627','628','629','630','631')), 0, 'X', 44
Union All
...
Where SUBSTRING(nocuenta ,1,4) In ('','')),
--Cambio a acumulado
(Select SUM(TotalAbonos) From @eAnterior  
Where SUBSTRING(nocuenta ,1,4) In ('','')), 0, 'X', 52
...
--Cambio a acumulado
(Select SUM(TotalCargos) From @eAnterior 
Where SUBSTRING(nocuenta ,1,4) In ('','')), 0, 'X', 59
...
(Select SaldoDeudor From @eActualPeriodo Where SUBSTRING(nocuenta ,1,4) = '1110'),
(Select SaldoDeudor From @eAnteriorPeriodo Where SUBSTRING(nocuenta ,1,4) = '1110')
, 1, 'X', 63
...
------ BR 20150814 -------------------------------------
UPDATE @report set SaldoActual=  (SELECT isnull(SUM(IMPORTEABONO),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE (MONTH(Fecha) Between @periodo and @periodoFin) AND Ejercicio = @EJERCICIO) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '81500-1%')
...
UPDATE @report set SaldoActual= ((SELECT isnull(SUM(IMPORTEABONO),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE (MONTH(Fecha) Between @periodo and @periodoFin) AND Ejercicio = @EJERCICIO) 
AND IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '81500-5%'))
...
UPDATE @report set SaldoAnterior=  (SELECT isnull(SUM(IMPORTEABONO),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE (MONTH(Fecha) Between @periodo and @periodoFin) AND Ejercicio = @EJERCICIO-1) AND 
IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '81500-1%')
) where nombre='IMPUESTOS' 
...
UPDATE @report set SaldoAnterior= ((SELECT isnull(SUM(IMPORTEABONO),0) FROM D_POLIZAS WHERE IDPOLIZA IN (SELECT IdPoliza FROM T_Polizas WHERE (MONTH(Fecha) Between @periodo and @periodoFin) AND Ejercicio = @EJERCICIO-1) 
AND IDCUENTACONTABLE IN (SELECT IDCUENTACONTABLE FROM C_CONTABLE WHERE NumeroCuenta LIKE '81500-5%'))
...
--Exec sp_rpt_flujo_de_efectivo2Redondeo 2017,1,1,0,0,0