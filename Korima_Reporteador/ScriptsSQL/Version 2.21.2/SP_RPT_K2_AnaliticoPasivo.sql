...
--exec SP_RPT_K2_AnaliticoPasivo 1,0,4,2015,1
...
if @Totales=1 Begin
if @MostarSinSaldo=1
SELECT * from @Tabla where NumeroCuenta ='2'+REPLICATE('0',@Estructura1-1)+'-'+@CerosEstructura ORDER BY NumeroCuenta
else
SELECT * from @Tabla where NumeroCuenta ='2'+REPLICATE('0',@Estructura1-1)+'-'+@CerosEstructura AND  (CargosSinFlujo <> 0 OR TotalCargos <> 0 OR TotalAbonos <> 0 OR SaldoFinal <> 0 OR FlujoDelperiodo <> 0) ORDER BY NumeroCuenta
end
else begin 
if @MostarSinSaldo=1
