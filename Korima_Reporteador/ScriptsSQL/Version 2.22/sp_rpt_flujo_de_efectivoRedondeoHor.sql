...
/****** Object:  StoredProcedure [dbo].[sp_rpt_flujo_def_efectivoHor]    Script Date: 12/03/2012 17:30:04 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_rpt_flujo_de_efectivoRedondeoHor]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[sp_rpt_flujo_de_efectivoRedondeoHor]
GO
CREATE PROCEDURE [dbo].[sp_rpt_flujo_de_efectivoRedondeoHor]
@ejercicio int, @periodo int, @miles Bit, @MostrarVacios bit,@Redondeo bit, @Tipo int
AS
...
-- DECLARE @report TABLE (nombre varchar(255), SaldoActual decimal(15,2), SaldoAnterior decimal(15,2), negritas tinyint, secciongroup char(1), orden decimal(15,2))
------ BR 20150814 -------------------------------------
UPDATE @report set SaldoActual= ( Select SUM(a.SaldoAcreedor) FROM @eActual a inner join @eAnterior b on a.nocuenta = b.nocuenta 
...
-- EXEC sp_rpt_flujo_de_efectivoRedondeo 2015,4,0,1,1