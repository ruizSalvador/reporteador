...
--exec SP_Ingresos_EgresosTrimestralTXT 0,2015,5
...
If @Tipo = 4
	Begin
		Select (Select COUNT(IdDefMeta) from T_DefinicionMetas Where Afectable = 1 and Ejercicio = @Ejercicio) as Programadas, (Select COUNT(Avance) from T_SeguimientoMeta Where Avance >= 100 and Year(Fecha)=@Ejercicio) as Cumplidas	
	End

If @Tipo = 5
	Begin
		Select Clave, CAST(SUM(Modificado) as Decimal(18,0)) Modificado, CAST(SUM(Recaudado) as Decimal(18,0)) Recaudado From T_PresupuestoFlujo tp
		INNER JOIN  C_PartidasGastosIngresos cp
		ON tp.IdPartida = cp.IdPartidaGI
		Where tp.Mes = 0 and tp.Ejercicio = @Ejercicio
		Group by Clave
		Order by Clave
	End
...
If @Tipo = 6
	Begin
		Select IdPartida, CAST(SUM(Modificado) as Decimal (18,0)) Modificado, CAST(SUM(Pagado) as Decimal(18,0)) Pagado from T_PresupuestoNW tp
		INNER JOIN T_SellosPresupuestales ts
		ON tp.IdSelloPresupuestal = ts.IdSelloPresupuestal
		WHere tp.Mes = 0  and Year = @Ejercicio
		Group by IdPartida
		Order by IdPartida
	End
END
