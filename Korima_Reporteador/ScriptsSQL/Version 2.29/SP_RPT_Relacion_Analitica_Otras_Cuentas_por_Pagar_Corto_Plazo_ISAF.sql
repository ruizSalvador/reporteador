IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_RPT_Relacion_Analitica_Otras_Cuentas_por_Pagar_Corto_Plazo_ISAF]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_RPT_Relacion_Analitica_Otras_Cuentas_por_Pagar_Corto_Plazo_ISAF]
GO
...
--EXEC SP_RPT_Relacion_Analitica_Otras_Cuentas_por_Pagar_Corto_Plazo_ISAF 1,6,2019;
CREATE PROCEDURE [dbo].[SP_RPT_Relacion_Analitica_Otras_Cuentas_por_Pagar_Corto_Plazo_ISAF]
@MesInicio smallint,    
@MesFin smallint,
@Ejercicio smallint
...
--AND SUBSTRING(CuentaAcumulacion,1,LEN('2119')) = '2119' AND Afectable != 0 
...
LEFT JOIN (Select C_Contable.IdCuentaContable, 
			...
			AND SUBSTRING(CuentaAcumulacion,1,LEN('2119')) = '2119' AND Afectable != 0 ) SaldosIniciales
...
LEFT JOIN (Select C_Contable.IdCuentaContable, 
			...
			AND SUBSTRING(CuentaAcumulacion,1,LEN('2119')) = '2119' AND Afectable != 0) SaldosFinales 
...
SELECT * FROM @Tabla WHERE (NumeroCuenta = '2119' OR SUBSTRING(NumeroCuenta,1,LEN('2119')) = '2119');

END

GO
EXEC SP_FirmasReporte 'Relacion Analitica Otras Cuentas por Pagar a Corto Plazo ISAF' 

GO

Exec SP_CFG_LogScripts 'SP_RPT_Relacion_Analitica_Otras_Cuentas_por_Pagar_Corto_Plazo_ISAF','2.29'
GO