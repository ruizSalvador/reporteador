...
OPEN Registros6
	...
	WHILE @@FETCH_STATUS = 0
		BEGIN
				 ...
				 INSERT INTO [dbo].[C_Contable]  VALUES ((select MAX(IdCuentaContable) from C_Contable) +1, @IdPartidaGIas, (SELECT STUFF(@NumeroCuenta, 1, 3, '821')), @NombreCuenta, (Select IdCuentaContable from C_Contable Where NumeroCuenta = ('821'+Replicate('0',@Estructura1-3)+'-'+@cerosEstructura)), 
			(SELECT STUFF(@CuentaAcumulacion, 1, 3, '821')), @TipoCuenta, @Afectable, @Nivel, @Alta, @Exportada, 
			@Financiero, @AfectaDisponibilidad, @IdSelloPresupuestal, @IdCuentaContableMayor, 
			@Utilizar, @Armonizado, @Observaciones, @SuSaldoRepresenta, @Descripcion, @Fecha, 
		    @SaldarCierreEjercicio, @CrearSubcuentasAutomaticas, @FechaAuditoria, @HoraAuditoria, @IdUsuarioModifica)			
...
Exec SP_CFG_LogScripts 'SP_CreaCuentaAutomaticas','2.29'
GO