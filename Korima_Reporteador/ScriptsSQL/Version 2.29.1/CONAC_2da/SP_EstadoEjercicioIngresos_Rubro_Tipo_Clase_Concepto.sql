...
--Exec SP_EstadoEjercicioIngresos_Rubro_Tipo_Clase_Concepto 1,1,1,2018,1,1
...
DECLARE @AcumAnual AS TABLE (Clasificacion varchar(255), Clave varchar(7), Estimado decimal(18,2), AmpliacionesReducciones decimal(18,2))
		...
		UNION
		SELECT C_Clas.Descripcion, C_Clas.Clave, SUM(Estimado) AS Estimado_Total, --C_Clas3.IdClasificacionGI, C_Clas3.IdClasificacionGIPadre
		SUM (isnull(Ampliaciones,0) )+ SUM (isnull(Reducciones,0)) AS AmpliacionesReducciones
		FROM T_PresupuestoFlujo TP RIGHT OUTER JOIN C_PartidasGastosIngresos TS ON TP.IdPartida  = TS.IdPartidaGI 
		INNER JOIN C_ClasificacionGasto C_Clas ON TS.IdClasificacionGI = C_Clas.IdClasificacionGI 
		INNER JOIN C_ClasificacionGasto C_Clas1
...
GO
Exec SP_CFG_LogScripts 'SP_EstadoEjercicioIngresos_Rubro_Tipo_Clase_Concepto','2.29'
GO
