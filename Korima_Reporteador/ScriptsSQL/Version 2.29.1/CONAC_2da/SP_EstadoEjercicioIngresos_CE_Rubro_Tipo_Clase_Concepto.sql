/****** Object:  StoredProcedure [dbo].[SP_EstadoEjercicioIngresos_CE_Rubro_Tipo_Clase_Concepto]    Script Date: 01/04/2013 16:10:36 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_EstadoEjercicioIngresos_CE_Rubro_Tipo_Clase_Concepto]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_EstadoEjercicioIngresos_CE_Rubro_Tipo_Clase_Concepto]
GO
...
-- EXEC SP_EstadoEjercicioIngresos_CE_Rubro_Tipo_Clase_Concepto 1,1,1,2018,1,1
CREATE PROCEDURE [dbo].[SP_EstadoEjercicioIngresos_CE_Rubro_Tipo_Clase_Concepto]
@MesInicio as int,
...
INSERT INTO @Resultado (Clave, Grupo1,Grupo2,Clasificacion,
Total_Estimado,Total_Modificado,Total_Devengado,Total_Recaudado, AmpliacionesReducciones)
Select 'I' as Clave,
 'I' as Grupo1, '' as Grupo2, 'INGRESOS CORRIENTES' as clasificacion,
...
Total_Estimado,Total_Modificado,Total_Devengado,Total_Recaudado,AmpliacionesReducciones)
Select 'II' as Clave,
'II' as Grupo1, '' as Grupo2, 'INGRESOS DE CAPITAL' as clasificacion,
...
Total_Estimado,Total_Modificado,Total_Devengado,Total_Recaudado,AmpliacionesReducciones)
Select 'III' as Clave,
'III' as Grupo1, '' as Grupo2, 'FINANCIAMIENTO' as clasificacion,
...
-- Red/Amp Acumuladas
DECLARE @AcumAnual AS TABLE (Clasificacion varchar(255), Clave varchar(200), Estimado decimal(15,2), AmpliacionesReducciones decimal(18,2), Modificado decimal(15,2))
...
--TIPO
SELECT     C_ClasificacionGasto_2.Descripcion AS Clasificacion,
			C_ClasificacionGasto_2.Clave AS Clave,
					  SUM(MovimientosPresupuesto.Estimado) AS Total_Estimado, 
					   SUM(isnull(MovimientosPresupuesto.Ampliaciones,0)) + SUM(isnull(MovimientosPresupuesto.Reducciones,0)) AS AmpliacionesReducciones,
					  SUM(MovimientosPresupuesto.Modificado) AS Total_Modificado
...
--CLASE                      
SELECT      C_ClasificacionGasto_1.Descripcion AS Clasificacion,
				C_ClasificacionGasto_1.Clave AS Clave,
					  SUM(MovimientosPresupuesto.Estimado) AS Total_Estimado, 
	                  SUM(isnull(MovimientosPresupuesto.Ampliaciones,0)) + SUM(isnull(MovimientosPresupuesto.Reducciones,0)) AS AmpliacionesReducciones,
					  SUM(MovimientosPresupuesto.Modificado) AS Total_Modificado
...
--Concepto
SELECT      C_ClasificacionGasto.Descripcion AS Clasificacion,
			C_ClasificacionGasto.Clave AS Clave,
					  SUM(MovimientosPresupuesto.Estimado) AS Total_Estimado,
					    SUM(isnull(MovimientosPresupuesto.Ampliaciones,0)) + SUM(isnull(MovimientosPresupuesto.Reducciones,0)) AS AmpliacionesReducciones, 
					  SUM(MovimientosPresupuesto.Modificado) AS Total_Modificado
...
GROUP BY C_ClasificacionGasto.Descripcion, C_ClasificacionGasto.Clave
		...
		INSERT INTO @AcumAnual (Clave, Clasificacion ,Estimado, Modificado, AmpliacionesReducciones)
		Select 'I' as Grupo1, 'INGRESOS CORRIENTES' as Clasificacion,
		(Select SUM (Estimado) from @AcumAnual where Clave in ('10000','20000','30000','40000','70000','80000','90000')) as Total_Estimado,
		(Select SUM (Modificado) from @AcumAnual where Clave in ('10000','20000','30000','40000','70000','80000','90000')) as Total_Modificado,
		(Select SUM (AmpliacionesReducciones) from @AcumAnual where Clave in ('10000','20000','30000','40000','70000','80000','90000')) as AmpliacionesReducciones

		INSERT INTO @AcumAnual (Clave, Clasificacion ,Estimado, Modificado, AmpliacionesReducciones)
		Select 'II' as Grupo1, 'INGRESOS DE CAPITAL' as Clasificacion,
		(Select SUM (Estimado) from @AcumAnual where Clave in ('50000','60000')) as Total_Estimado,
		(Select SUM (Modificado) from @AcumAnual where Clave in ('50000','60000')) as Total_Modificado,
		(Select SUM (AmpliacionesReducciones) from @AcumAnual where Clave in ('50000','60000')) as AmpliacionesReducciones
...
IF @AprAnual = 1
	...
	Insert into @Resultado2
		Select distinct A.Clave, A.Clasificacion,
		ISNULL(A.Estimado,0) as Total_Estimado,
		ISNULL(B.Total_Modificado,0) as Total_Modificado,
		ISNULL(B.Total_Devengado,0) as Total_Devengado,	
		ISNULL(B.Total_Recaudado,0) as Total_Recaudado,
		Grupo1,
		Grupo2,
		Grupo3,
		Grupo4,
		CASE @AmpRedAnual
		WHEN 1 THEN ISNULL(A.AmpliacionesReducciones,0)
		ELSE ISNULL(B.AmpliacionesReducciones,0)
		END as AmpliacionesReducciones,
		ISNULL(B.Excedentes,0) as Excedentes
		FROM @AcumAnual A LEFT JOIN @Resultado B ON A.Clave = B.Clave and A.Clasificacion = B.Clasificacion
		Order by Clave
	END
ELSE
BEGIN 
...
IF @MostrarVacios=1
	BEGIN
		...
		isnull((Select Sum(Total_Modificado)- Sum(Total_Recaudado) from @Resultado2 where Grupo2 like '_0'),0) as SumaPorRecaudar,
		isnull((Select Sum(Total_Estimado)from @Resultado2 where Grupo2 like '_0'),0) as SumaEstimado,
		isnull((Select Sum(Total_Modificado)from @Resultado2 where Grupo2 like '_0'),0) as SumaModificado,
		isnull((Select Sum(Total_Devengado)from @Resultado2 where Grupo2 like '_0'),0) as SumaDevengado,
		isnull((Select Sum(Total_Recaudado)from @Resultado2 where Grupo2 like '_0'),0) as SumaRecaudado,
		isnull((Select SUM(Total_Modificado)/nullif(SUM(Total_Estimado),0) from @Resultado2 where Grupo2 like '_0'),0) as ResModificado,
		isnull((Select SUM(Total_Devengado)/nullif(SUM(Total_Estimado),0) from @Resultado2 where Grupo2 like '_0'),0)as ResDevengado,
		isnull((Select SUM(Total_Recaudado)/nullif((SUM(Total_Estimado)+SUM(AmpliacionesReducciones)),0) from @Resultado2 where Grupo2 like '_0' ),0) as ResRecaudado,
		...
		isnull((Select Sum(AmpliacionesReducciones)from @Resultado2 where Grupo2 like '_0'),0) as SumaAmpliacionesReducciones, 
		isnull((Select SUM(isnull(Total_Recaudado,0))-sum(isnull(Total_Estimado,0)) from @Resultado2 where Grupo2 like '_0'),0)as SumaExcedentes 
		FROM @Resultado2
		--Group by Clave, Clasificacion
...
GO
UPDATE C_Menu SET Utilizar=1 WHERE IdMenu in(1124)
GO
