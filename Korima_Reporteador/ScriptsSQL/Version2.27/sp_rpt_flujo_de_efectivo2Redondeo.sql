...
-- Exec sp_rpt_flujo_de_efectivo2Redondeo 2016,1,11,0,1,0
...
From T_SaldosInicialesCont Where ( Mes = @PeriodoFin) And [Year] = @ejercicio group by IdCuentaContable,Year,Mes) Saldos
...
(Select ISNULL(SUM(0),0) as TotalAbonos from @eActual Where SUBSTRING(nocuenta ,1,4) = '8150'
And (SUBSTRING(nocuenta ,CHARINDEX('-', nocuenta) + 1 ,3) In ( '523')OR SUBSTRING(nocuenta ,CHARINDEX('-', nocuenta) + 1 ,4) In ('0524', '0523'))),
(Select ISNULL(SUM(0),0) as TotalAbonos from @eAnterior Where SUBSTRING(nocuenta ,1,4) = '8150'
...
Exec SP_CFG_LogScripts 'sp_rpt_flujo_de_efectivo2Redondeo'
GO
...
GO
 ...
 --Exec sp_rpt_flujo_de_efectivo2Redondeo 2017,1,3,0,1,0