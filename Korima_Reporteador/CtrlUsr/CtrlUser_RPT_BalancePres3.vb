...
Public Class CtrlUser_RPT_BalancePres3
    Dim TipoReporte
    ...
    Public reporte As RPT_BalancePres3
    ...
    Private Sub SimpleButton1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SimpleButton1.Click
        ...
        Dim command As New SqlCommand("SP_RPT_BalancePresupuestario", conection)
        command.CommandType = CommandType.StoredProcedure
        command.Parameters.Add(prmEjercicio)
        command.Parameters.Add(prmPeriodo)
        command.Parameters.Add(prmPeriodo2)
        command.Parameters.Add(miles)
        Dim adapter As New SqlDataAdapter(command)
        Dim ds As New DataSet()
        ds.EnforceConstraints = False
        adapter.Fill(ds, "SP_RPT_BalancePresupuestario")
        reporte.DataSource = ds

        reporte.DataAdapter = adapter
        reporte.DataMember = "SP_RPT_BalancePresupuestario"

        'Firmas 
        Dim adapterC As SqlClient.SqlDataAdapter
        adapterC = New SqlClient.SqlDataAdapter("SELECT * FROM VW_RPT_K2_Firmas2 where Formato = '" & "LDF Balance Presupuestario" & "' and (Nombre1 <>'' or Puesto1 <> '' or Nombre2 <>'' or Puesto2<>'') Order by Orden ", cnnString)
        ...
        With reporte
            ...
            Dim cmd As New SqlCommand("SELECT TOP(1) T_Firmas.CodigoISO FROM C_Formatos JOIN T_Firmas ON C_Formatos.IdFormato = T_Firmas.IdFormato  Where Formato='LDF Balance Presupuestario' ", New SqlConnection(cnnString))
            cmd.Connection.Open()
    ...
    Private Sub BarLargeButtonItem1_ItemClick(sender As System.Object, e As DevExpress.XtraBars.ItemClickEventArgs)
        ...
        dlg.Title = "Exporta Balance Presupuestario"
    ...
    Public Sub BarButtonItem4_ItemClick(ByVal sender As System.Object, ByVal e As DevExpress.XtraBars.ItemClickEventArgs)
        ...
        frm.Text = "Balance Presupuestario"
            ...
            For Each dr As DataRow In frm.dsNotas.Tables(0).Rows
                For j As Integer = i To ds.Tables(0).Rows.Count
                    If ds.Tables(0).Rows(j)("Concepto") = dr("Concepto") Then
                        ds.Tables(0).Rows(j)("Estimado") = dr("Estimado")
                        ds.Tables(0).Rows(j)("Devengado") = dr("Devengado")
                        ds.Tables(0).Rows(j)("Recaudado") = dr("Recaudado")
